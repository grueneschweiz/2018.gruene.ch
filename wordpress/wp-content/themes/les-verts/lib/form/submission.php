...
class FormSubmission {
	...
	public function handle_submit() {
		...
		if ( !(isset($_REQUEST['_supt_action']) && strpos($_REQUEST['_supt_action'], self::ACTION_BASE_NAME) !== false) ) {
			return;
		}
		...
		if ( in_array( $sendMethod, array('email', 'news')) ) {
			...
			try {
				// Notification
				$sent1 = supt_form_send_email( $email['to'], $email['from'], $email['notif']['subject'], $email['notif']['template'], $this->data );

				// Auto Reply
				$sent2 = true;
				if ( isset($this->autoReplyEmail) ) {
					$sent2 = supt_form_send_email( $this->autoReplyEmail, $email['from'], $email['autoreply']['subject'], $email['autoreply']['template'], $this->data );
				}
			}
			catch (Exception $e) {
	...
	private function filterSanitizeValidate() {
		...
		if ( !is_array($conf_fields) ) $conf_fields = array();
	...
	private function getEmailParams() {
		...
		if ( !empty($fromName) )	$from = "$fromName <$fromEmail>";
		else											$from = $fromEmail;
