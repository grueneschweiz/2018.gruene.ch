...
class FormType extends Model {
	...
	public static function register_acf_fields() {
		...
		add_filter( 'acf/prepare_field/name=form_email_from',
			array( __CLASS__, 'acf_populate_placeholder_local_settings' ) );
		add_filter( 'acf/prepare_field/name=form_name_from',
			array( __CLASS__, 'acf_populate_placeholder_local_settings' ) );
		add_filter( 'acf/prepare_field/name=form_notif_subject',
			array( __CLASS__, 'acf_populate_placeholder_local_settings' ) );
		add_filter( 'acf/prepare_field/name=form_notif_template',
			array( __CLASS__, 'acf_populate_placeholder_local_settings' ) );
		add_filter( 'acf/prepare_field/name=form_autoreply_subject',
			array( __CLASS__, 'acf_populate_placeholder_local_settings' ) );
		add_filter( 'acf/prepare_field/name=form_autoreply_template',
			array( __CLASS__, 'acf_populate_placeholder_local_settings' ) );
		add_filter( 'acf/prepare_field/name=form_privacy_page',
			array( __CLASS__, 'acf_populate_placeholder_local_settings' ) );
		...
		add_filter( 'acf/prepare_field/name=form_autoreply_subject',
			array( __CLASS__, 'acf_remove_default_local_settings' ) );
		add_filter( 'acf/prepare_field/name=form_autoreply_template',
			array( __CLASS__, 'acf_remove_default_local_settings' ) );
		...
		add_filter( 'acf/validate_value/name=form_email_to', array( __CLASS__, 'acf_validate_empty_value_local_settings' ),
			20, 2 );
		add_filter( 'acf/validate_value/name=form_email_from',
			array( __CLASS__, 'acf_validate_empty_value_local_settings' ), 20, 2 );
		add_filter( 'acf/validate_value/name=form_name_from', array( __CLASS__, 'acf_validate_empty_value_local_settings' ),
			20, 2 );
		add_filter( 'acf/validate_value/name=form_notif_subject',
			array( __CLASS__, 'acf_validate_empty_value_local_settings' ), 20, 2 );
		add_filter( 'acf/validate_value/name=form_notif_template',
			array( __CLASS__, 'acf_validate_empty_value_local_settings' ), 20, 2 );
		add_filter( 'acf/validate_value/name=form_autoreply_subject',
			array( __CLASS__, 'acf_validate_empty_value_local_settings' ), 20, 2 );
		add_filter( 'acf/validate_value/name=form_autoreply_template',
			array( __CLASS__, 'acf_validate_empty_value_local_settings' ), 20, 2 );
		add_filter( 'acf/validate_value/name=form_privacy_page',
			array( __CLASS__, 'acf_validate_empty_value_local_settings' ), 20, 2 );
	}
	...
	public static function register_actions_filters() {
		...
		add_action( 'manage_' . self::MODEL_NAME . '_posts_custom_column', array( __CLASS__, 'populate_columns_fields' ),
			10, 2 );
	...
	public static function add_sent_page_menu() {
		if ( get_field( 'form_save_db', 'options' ) ) {
			add_submenu_page( 'edit.php?post_type=theme_form', __( 'Form submissions', THEME_DOMAIN ),
