<?php

/**
 * The only goal of this file, is to expose the acf translations to gettext
 */

die();

if ( function_exists( 'acf_add_local_field_group' ) ):

	acf_add_local_field_group( array(
		'key'                   => 'group_5fa587aac61ec',
		'title'                 => __( '(Category) Author', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5fa587ab352a8',
				'label'             => __( 'Show author', 'lesverts' ),
				'name'              => 'has_author',
				'aria-label'        => '',
				'type'              => 'true_false',
				'instructions'      => __( 'Show the author for all posts of this category?', 'lesverts' ),
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'message'           => '',
				'default_value'     => 0,
				'ui'                => 1,
				'ui_on_text'        => '',
				'ui_off_text'       => '',
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'taxonomy',
					'operator' => '==',
					'value'    => 'category',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => true,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1604683888,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5c5ab079896c0',
		'title'                 => __( '(Category) Date', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5c5ab09d6c3f1',
				'label'             => __( 'Has date', 'lesverts' ),
				'name'              => 'has_date',
				'aria-label'        => '',
				'type'              => 'true_false',
				'instructions'      => __( 'Should a date be displayed with all posts categorized in this category?', 'lesverts' ),
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'message'           => '',
				'default_value'     => 0,
				'ui'                => 1,
				'ui_on_text'        => '',
				'ui_off_text'       => '',
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'taxonomy',
					'operator' => '==',
					'value'    => 'category',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => true,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1549460588,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b9ba36e445dd',
		'title'                 => __( '(Event) Venue', 'lesverts' ),
		'fields'                => array(),
		'location'              => array(
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'tribe_venue',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'seamless',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => array(
			0  => 'the_content',
			1  => 'excerpt',
			2  => 'discussion',
			3  => 'comments',
			4  => 'revisions',
			5  => 'slug',
			6  => 'author',
			7  => 'format',
			8  => 'page_attributes',
			9  => 'featured_image',
			10 => 'categories',
			11 => 'tags',
			12 => 'send-trackbacks',
		),
		'active'                => true,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1543324738,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5bd05fb159df1',
		'title'                 => __( '(Media) Focal point', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5bd06039df32d',
				'label'             => __( 'Focal point', 'lesverts' ),
				'name'              => 'focal_point',
				'aria-label'        => '',
				'type'              => 'select',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'choices'           => array(
					'top-left'      => __( 'top left', 'lesverts' ),
					'top-center'    => __( 'top center', 'lesverts' ),
					'top-right'     => __( 'top right', 'lesverts' ),
					'middle-left'   => __( 'middle left', 'lesverts' ),
					'middle-center' => __( 'middle center', 'lesverts' ),
					'middle-right'  => __( 'middle right', 'lesverts' ),
					'bottom-left'   => __( 'bottom left', 'lesverts' ),
					'bottom-center' => __( 'bottom center', 'lesverts' ),
					'bottom-right'  => __( 'bottom right', 'lesverts' ),
				),
				'default_value'     => 'middle-center',
				'allow_null'        => 0,
				'multiple'          => 0,
				'ui'                => 0,
				'return_format'     => 'value',
				'ajax'              => 0,
				'placeholder'       => '',
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'attachment',
					'operator' => '==',
					'value'    => 'image/jpeg',
				),
			),
			array(
				array(
					'param'    => 'attachment',
					'operator' => '==',
					'value'    => 'image/png',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => true,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1549396371,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b647ca5e8da4',
		'title'                 => __( '(Media) Image Copyright', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5b647cb444899',
				'label'             => __( 'Copyright Info', 'lesverts' ),
				'name'              => 'copyright',
				'aria-label'        => '',
				'type'              => 'text',
				'instructions'      => __( 'If you add one of the following licenses (p.ex: <emph>Tunick Spencer (CC BY)</emph>), we\'ll automatically add a link to the license. Recognized licenses:
<ul style="display: inline">
<li style="display: inline"><a href="https://creativecommons.org/publicdomain/zero/1.0/">CC0</a></li>
<li style="display: inline"><a href="https://creativecommons.org/licenses/by/4.0/">CC BY</a></li>
<li style="display: inline"><a href="https://creativecommons.org/licenses/by-sa/4.0/">CC BY-SA</a></li>
<li style="display: inline"><a href="https://creativecommons.org/licenses/by-nc/4.0/">CC BY-NC</a></li>
<li style="display: inline"><a href="https://creativecommons.org/licenses/by-nc-sa/4.0/">CC BY-NC-SA</a></li>
<li style="display: inline"><a href="https://creativecommons.org/licenses/by-nd/4.0/">CC BY-ND</a></li>
<li style="display: inline"><a href="https://creativecommons.org/licenses/by-nc-nd/4.0/">CC BY-NC-ND</a></li>
</ul>', 'lesverts' ),
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'default_value'     => '',
				'placeholder'       => '',
				'prepend'           => '',
				'append'            => '',
				'maxlength'         => '',
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'attachment',
					'operator' => '==',
					'value'    => 'image',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => true,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1570648072,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b62cd118cbad',
		'title'                 => __( '(Menu) Featured Subnav Item', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5b62da7660cb3',
				'label'             => __( 'Featured Menu Item', 'lesverts' ),
				'name'              => 'featured_menu_item',
				'aria-label'        => '',
				'type'              => 'true_false',
				'instructions'      => __( 'This option only works on sub items (level 2). Else it won\'t have any effect.', 'lesverts' ),
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'message'           => '',
				'default_value'     => 0,
				'ui'                => 1,
				'ui_on_text'        => '',
				'ui_off_text'       => '',
			),
			array(
				'key'               => 'field_5b730997902d0',
				'label'             => '',
				'name'              => 'group',
				'aria-label'        => '',
				'type'              => 'group',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => array(
					array(
						array(
							'field'    => 'field_5b62da7660cb3',
							'operator' => '==',
							'value'    => '1',
						),
					),
				),
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'layout'            => 'block',
				'sub_fields'        => array(
					array(
						'key'               => 'field_5b71c942afd74',
						'label'             => __( 'Block Title', 'lesverts' ),
						'name'              => 'block_title',
						'aria-label'        => '',
						'type'              => 'clone',
						'instructions'      => '',
						'required'          => 1,
						'conditional_logic' => array(
							array(
								array(
									'field'    => 'field_5b62da7660cb3',
									'operator' => '==',
									'value'    => '1',
								),
							),
						),
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'clone'             => array(
							0 => 'group_5b684e748247e',
						),
						'display'           => 'seamless',
						'layout'            => 'block',
						'prefix_label'      => 0,
						'prefix_name'       => 0,
					),
					array(
						'key'               => 'field_5b62ceb8766b1',
						'label'             => __( 'Image', 'lesverts' ),
						'name'              => 'image',
						'aria-label'        => '',
						'type'              => 'image',
						'instructions'      => '',
						'required'          => 1,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'return_format'     => 'id',
						'preview_size'      => 'regular',
						'library'           => 'all',
						'min_width'         => 538,
						'min_height'        => '',
						'min_size'          => '',
						'max_width'         => '',
						'max_height'        => '',
						'max_size'          => '',
						'mime_types'        => '',
					),
					array(
						'key'               => 'field_5b62cf84766b4',
						'label'             => __( 'Teaser', 'lesverts' ),
						'name'              => 'teaser',
						'aria-label'        => '',
						'type'              => 'textarea',
						'instructions'      => __( 'Max. 140 characters.', 'lesverts' ),
						'required'          => 0,
						'conditional_logic' => array(
							array(
								array(
									'field'    => 'field_5b62da7660cb3',
									'operator' => '==',
									'value'    => '1',
								),
							),
						),
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'default_value'     => '',
						'placeholder'       => '',
						'maxlength'         => 140,
						'rows'              => 4,
						'new_lines'         => '',
					),
					array(
						'key'               => 'field_5b62cfc0766b5',
						'label'             => __( 'Button Text', 'lesverts' ),
						'name'              => 'button_text',
						'aria-label'        => '',
						'type'              => 'text',
						'instructions'      => __( 'The button will link to this menu item', 'lesverts' ),
						'required'          => 0,
						'conditional_logic' => array(
							array(
								array(
									'field'    => 'field_5b62da7660cb3',
									'operator' => '==',
									'value'    => '1',
								),
							),
						),
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'default_value'     => 'Show campaign',
						'placeholder'       => '',
						'prepend'           => '',
						'append'            => '',
						'maxlength'         => 60,
					),
				),
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'nav_menu_item',
					'operator' => '==',
					'value'    => 'all',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => true,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1635972087,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b6d6d4f568ab',
		'title'                 => __( '(Widget) Buttons', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5b6d6d7d49392',
				'label'             => __( 'Buttons', 'lesverts' ),
				'name'              => 'buttons',
				'aria-label'        => '',
				'type'              => 'repeater',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'collapsed'         => 'field_5b6d6de749393',
				'min'               => 1,
				'max'               => 5,
				'layout'            => 'block',
				'button_label'      => __( 'Add Button', 'lesverts' ),
				'sub_fields'        => array(
					array(
						'key'               => 'field_5b6d6de749393',
						'label'             => __( 'Button Text', 'lesverts' ),
						'name'              => 'label',
						'aria-label'        => '',
						'type'              => 'text',
						'instructions'      => __( 'Max. 40 characters.', 'lesverts' ),
						'required'          => 1,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'default_value'     => '',
						'placeholder'       => '',
						'prepend'           => '',
						'append'            => '',
						'maxlength'         => 40,
						'parent_repeater'   => 'field_5b6d6d7d49392',
					),
					array(
						'key'               => 'field_5b6d6dfa49394',
						'label'             => __( 'Link', 'lesverts' ),
						'name'              => 'link',
						'aria-label'        => '',
						'type'              => 'url',
						'instructions'      => '',
						'required'          => 1,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'default_value'     => '',
						'placeholder'       => '',
						'parent_repeater'   => 'field_5b6d6d7d49392',
					),
					array(
						'key'               => 'field_5b6d6e1349395',
						'label'             => __( 'Highlighted', 'lesverts' ),
						'name'              => 'highlighted',
						'aria-label'        => '',
						'type'              => 'true_false',
						'instructions'      => '',
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'message'           => '',
						'default_value'     => 0,
						'ui'                => 1,
						'ui_on_text'        => '',
						'ui_off_text'       => '',
						'parent_repeater'   => 'field_5b6d6d7d49392',
					),
				),
				'rows_per_page'     => 20,
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'widget',
					'operator' => '==',
					'value'    => 'supt_button_widget',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => true,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1543324739,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b6d603e15f2a',
		'title'                 => __( '(Widget) Contact', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5b6d66726dae9',
				'label'             => __( 'Address', 'lesverts' ),
				'name'              => 'address',
				'aria-label'        => '',
				'type'              => 'wysiwyg',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'default_value'     => '',
				'tabs'              => 'all',
				'toolbar'           => 'full',
				'media_upload'      => 0,
				'delay'             => 0,
			),
			array(
				'key'               => 'field_5b6d66eb6daeb',
				'label'             => __( 'Social Media', 'lesverts' ),
				'name'              => 'social_media',
				'aria-label'        => '',
				'type'              => 'flexible_content',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'layouts'           => array(
					'5b6d66ff18a3e'        => array(
						'key'        => '5b6d66ff18a3e',
						'name'       => 'facebook',
						'label'      => __( 'Facebook', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b6d67a21c8de',
								'label'             => __( 'Link to the facebook fan page / profile', 'lesverts' ),
								'name'              => 'link',
								'aria-label'        => '',
								'type'              => 'url',
								'instructions'      => '',
								'required'          => 1,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'placeholder'       => '',
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_5b6d67df1c8df' => array(
						'key'        => 'layout_5b6d67df1c8df',
						'name'       => 'twitter',
						'label'      => __( 'Twitter', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b6d67e81c8e0',
								'label'             => __( 'Link to the twitter profile', 'lesverts' ),
								'name'              => 'link',
								'aria-label'        => '',
								'type'              => 'url',
								'instructions'      => '',
								'required'          => 1,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'placeholder'       => '',
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_63d42064033f8' => array(
						'key'        => 'layout_63d42064033f8',
						'name'       => 'mastodon',
						'label'      => __( 'Mastodon', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_63d42064033f9',
								'label'             => __( 'Link to the mastodon profile', 'lesverts' ),
								'name'              => 'link',
								'aria-label'        => '',
								'type'              => 'url',
								'instructions'      => '',
								'required'          => 1,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'translations'      => 'copy_once',
								'placeholder'       => '',
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_63d42127948a1' => array(
						'key'        => 'layout_63d42127948a1',
						'name'       => 'tumblr',
						'label'      => __( 'Tumblr', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_63d42127948a2',
								'label'             => __( 'Link to the tumblr profile', 'lesverts' ),
								'name'              => 'link',
								'aria-label'        => '',
								'type'              => 'url',
								'instructions'      => '',
								'required'          => 1,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'translations'      => 'copy_once',
								'placeholder'       => '',
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_5b6d680d1c8e1' => array(
						'key'        => 'layout_5b6d680d1c8e1',
						'name'       => 'instagram',
						'label'      => __( 'Instagram', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b6d681c1c8e2',
								'label'             => __( 'Link to the instagram profile', 'lesverts' ),
								'name'              => 'link',
								'aria-label'        => '',
								'type'              => 'url',
								'instructions'      => '',
								'required'          => 1,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'placeholder'       => '',
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_threads_frontend' => array(
						'key'        => 'layout_threads_frontend',
						'name'       => 'threads',
						'label'      => __( 'Threads', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_threads_frontend',
								'label'             => __( 'Link to the threads profile', 'lesverts' ),
								'name'              => 'link',
								'aria-label'        => '',
								'type'              => 'url',
								'instructions'      => '',
								'required'          => 1,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'placeholder'       => '',
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_tiktok_frontend' => array(
						'key'        => 'layout_tiktok_frontend',
						'name'       => 'tiktok',
						'label'      => __( 'Tiktok', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_tiktok_frontend',
								'label'             => __( 'Link to the tiktok profile', 'lesverts' ),
								'name'              => 'link',
								'aria-label'        => '',
								'type'              => 'url',
								'instructions'      => '',
								'required'          => 1,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'placeholder'       => '',
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_bluesky_frontend' => array(
						'key'        => 'layout_bluesky_frontend',
						'name'       => 'bluesky',
						'label'      => __( 'Bluesky', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_bluesky_frontend',
								'label'             => __( 'Link to the Bluesky profile', 'lesverts' ),
								'name'              => 'link',
								'aria-label'        => '',
								'type'              => 'url',
								'instructions'      => '',
								'required'          => 1,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'placeholder'       => '',
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_626bcda0bb4b4' => array(
						'key'        => 'layout_626bcda0bb4b4',
						'name'       => 'linkedin',
						'label'      => __( 'Linkedin', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_626bcda0bb4b5',
								'label'             => __( 'Link to the linkedin profile', 'lesverts' ),
								'name'              => 'link',
								'aria-label'        => '',
								'type'              => 'url',
								'instructions'      => '',
								'required'          => 1,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'placeholder'       => '',
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_626bcdb7bb4b6' => array(
						'key'        => 'layout_626bcdb7bb4b6',
						'name'       => 'youtube',
						'label'      => __( 'Youtube', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_626bcdb7bb4b7',
								'label'             => __( 'Link to the youtube profile', 'lesverts' ),
								'name'              => 'link',
								'aria-label'        => '',
								'type'              => 'url',
								'instructions'      => '',
								'required'          => 1,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'placeholder'       => '',
							),
						),
						'min'        => '',
						'max'        => '1',
					),
				),
				'min'               => '',
				'max'               => '',
				'button_label'      => __( 'Add Icon', 'lesverts' ),
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'widget',
					'operator' => '==',
					'value'    => 'supt_contact_widget',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => true,
		'description'           => '',
		'show_in_rest'          => 0,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b6d9c4c1eaa5',
		'title'                 => __( '(Widget) Link List', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5b6d9c573efbd',
				'label'             => __( 'List', 'lesverts' ),
				'name'              => 'list',
				'aria-label'        => '',
				'type'              => 'repeater',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'collapsed'         => 'field_5b6d9c713efbe',
				'min'               => 0,
				'max'               => 0,
				'layout'            => 'row',
				'button_label'      => __( 'Add Link', 'lesverts' ),
				'sub_fields'        => array(
					array(
						'key'               => 'field_5b6d9c713efbe',
						'label'             => __( 'Label', 'lesverts' ),
						'name'              => 'label',
						'aria-label'        => '',
						'type'              => 'text',
						'instructions'      => '',
						'required'          => 1,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'default_value'     => '',
						'placeholder'       => '',
						'prepend'           => '',
						'append'            => '',
						'maxlength'         => '',
						'parent_repeater'   => 'field_5b6d9c573efbd',
					),
					array(
						'key'               => 'field_5b6d9c923efbf',
						'label'             => __( 'Link', 'lesverts' ),
						'name'              => 'link',
						'aria-label'        => '',
						'type'              => 'url',
						'instructions'      => '',
						'required'          => 1,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'default_value'     => '',
						'placeholder'       => '',
						'parent_repeater'   => 'field_5b6d9c573efbd',
					),
				),
				'rows_per_page'     => 20,
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'widget',
					'operator' => '==',
					'value'    => 'supt_link_list_widget',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => true,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1543324740,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5fdbe45d039e4',
		'title'                 => __( '[Template]Block', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5fdbe46eb2e43',
				'label'             => __( 'About this Block', 'lesverts' ),
				'name'              => '',
				'aria-label'        => '',
				'type'              => 'message',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'message'           => __( 'You want to share content over multiple pages? Create the content as block post type, then pull it in here.', 'lesverts' ),
				'new_lines'         => 'wpautop',
				'esc_html'          => 0,
			),
			array(
				'key'               => 'field_5fdbe48cb2e44',
				'label'             => __( 'Block', 'lesverts' ),
				'name'              => 'block',
				'aria-label'        => '',
				'type'              => 'post_object',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'post_type'         => array(
					0 => 'block',
				),
				'taxonomy'          => '',
				'allow_null'        => 0,
				'multiple'          => 0,
				'return_format'     => 'object',
				'translations'      => 'copy_once',
				'ui'                => 1,
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'post',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => false,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1608247019,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b684e748247e',
		'title'                 => __( '[Template]Block title', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5b7199bdd9d92',
				'label'             => __( 'Block Title', 'lesverts' ),
				'name'              => 'block_title',
				'aria-label'        => '',
				'type'              => 'group',
				'instructions'      => __( 'Use a generic title like the category of the block. Then link it to this category.', 'lesverts' ),
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'layout'            => 'row',
				'sub_fields'        => array(
					array(
						'key'               => 'field_5b6860ce304b8',
						'label'             => __( 'Title', 'lesverts' ),
						'name'              => 'title',
						'aria-label'        => '',
						'type'              => 'text',
						'instructions'      => '',
						'required'          => 1,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'default_value'     => '',
						'placeholder'       => '',
						'prepend'           => '',
						'append'            => '',
						'maxlength'         => 80,
					),
					array(
						'key'               => 'field_5b719aeeac444',
						'label'             => __( 'Link', 'lesverts' ),
						'name'              => 'link',
						'aria-label'        => '',
						'type'              => 'group',
						'instructions'      => '',
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'layout'            => 'table',
						'sub_fields'        => array(
							array(
								'key'               => 'field_5b71975ea470c',
								'label'             => __( 'Link Type', 'lesverts' ),
								'name'              => 'type',
								'aria-label'        => '',
								'type'              => 'button_group',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '25',
									'class' => '',
									'id'    => '',
								),
								'choices'           => array(
									'category' => __( 'Category', 'lesverts' ),
									'internal' => __( 'Internal', 'lesverts' ),
									'url'      => __( 'URL', 'lesverts' ),
									'none'     => __( 'None', 'lesverts' ),
								),
								'allow_null'        => 0,
								'default_value'     => 'none',
								'layout'            => 'vertical',
								'return_format'     => 'value',
							),
							array(
								'key'               => 'field_5b6860fd304b9',
								'label'             => __( 'Category', 'lesverts' ),
								'name'              => 'category',
								'aria-label'        => '',
								'type'              => 'taxonomy',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => array(
									array(
										array(
											'field'    => 'field_5b71975ea470c',
											'operator' => '==',
											'value'    => 'category',
										),
									),
								),
								'wrapper'           => array(
									'width' => '75',
									'class' => '',
									'id'    => '',
								),
								'taxonomy'          => 'category',
								'field_type'        => 'select',
								'allow_null'        => 0,
								'add_term'          => 0,
								'save_terms'        => 0,
								'load_terms'        => 0,
								'return_format'     => 'id',
								'multiple'          => 0,
							),
							array(
								'key'               => 'field_5b71984fa470e',
								'label'             => __( 'Internal Link', 'lesverts' ),
								'name'              => 'internal',
								'aria-label'        => '',
								'type'              => 'post_object',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => array(
									array(
										array(
											'field'    => 'field_5b71975ea470c',
											'operator' => '==',
											'value'    => 'internal',
										),
									),
								),
								'wrapper'           => array(
									'width' => '75',
									'class' => '',
									'id'    => '',
								),
								'post_type'         => array(
									0 => 'post',
									1 => 'page',
									2 => 'tribe_events',
								),
								'taxonomy'          => array(),
								'allow_null'        => 0,
								'multiple'          => 0,
								'return_format'     => 'object',
								'ui'                => 1,
							),
							array(
								'key'               => 'field_5b7198bba470f',
								'label'             => __( 'URL', 'lesverts' ),
								'name'              => 'url',
								'aria-label'        => '',
								'type'              => 'url',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => array(
									array(
										array(
											'field'    => 'field_5b71975ea470c',
											'operator' => '==',
											'value'    => 'url',
										),
									),
								),
								'wrapper'           => array(
									'width' => '75',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'placeholder'       => '',
							),
							array(
								'key'               => 'field_5b719bb003bf4',
								'label'             => __( 'No Link', 'lesverts' ),
								'name'              => '',
								'aria-label'        => '',
								'type'              => 'message',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => array(
									array(
										array(
											'field'    => 'field_5b71975ea470c',
											'operator' => '==',
											'value'    => 'none',
										),
									),
								),
								'wrapper'           => array(
									'width' => '75',
									'class' => '',
									'id'    => '',
								),
								'message'           => __( 'The block title will not be linked', 'lesverts' ),
								'new_lines'         => 'wpautop',
								'esc_html'          => 0,
							),
						),
					),
				),
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'page_type',
					'operator' => '==',
					'value'    => 'front_page',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => false,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1543324740,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5cee958e0d362',
		'title'                 => __( '[Template]Default Content', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5cee95b5caff7',
				'label'             => __( 'Content', 'lesverts' ),
				'name'              => 'content',
				'aria-label'        => '',
				'type'              => 'flexible_content',
				'instructions'      => __( 'This is the main section of this page.', 'lesverts' ),
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'layouts'           => array(
					'5b6c41335aefe'        => array(
						'key'        => '5b6c41335aefe',
						'name'       => 'in_short',
						'label'      => __( 'In Short', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b6c49f24c11d',
								'label'             => __( 'In Short', 'lesverts' ),
								'name'              => 'in_short',
								'aria-label'        => '',
								'type'              => 'clone',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'clone'             => array(
									0 => 'group_5b6c416ac235f',
								),
								'display'           => 'seamless',
								'layout'            => 'block',
								'prefix_label'      => 0,
								'prefix_name'       => 0,
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_5b6c4a2a4c11e' => array(
						'key'        => 'layout_5b6c4a2a4c11e',
						'name'       => 'quote',
						'label'      => __( 'Quote', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b6c4a324c11f',
								'label'             => __( 'Quote', 'lesverts' ),
								'name'              => 'quote',
								'aria-label'        => '',
								'type'              => 'clone',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'clone'             => array(
									0 => 'group_5b6c045f68e96',
								),
								'display'           => 'seamless',
								'layout'            => 'block',
								'prefix_label'      => 0,
								'prefix_name'       => 0,
							),
						),
						'min'        => '',
						'max'        => '',
					),
					'layout_5b6c4a5f4c120' => array(
						'key'        => 'layout_5b6c4a5f4c120',
						'name'       => 'text',
						'label'      => __( 'Text', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b7d9304041de',
								'label'             => __( 'About this Block', 'lesverts' ),
								'name'              => '',
								'aria-label'        => '',
								'type'              => 'message',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'message'           => __( 'Use this block to add text. If you need to add raw HTML, make sure you use the \'text\' mode instead of the \'visual\' mode.', 'lesverts' ),
								'new_lines'         => 'wpautop',
								'esc_html'          => 0,
							),
							array(
								'key'               => 'field_5b6c4a7a4c121',
								'label'             => '',
								'name'              => 'text',
								'aria-label'        => '',
								'type'              => 'wysiwyg',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'tabs'              => 'all',
								'toolbar'           => 'full',
								'media_upload'      => 1,
								'delay'             => 0,
							),
						),
						'min'        => '',
						'max'        => '',
					),
					'layout_5b6c4aa24c122' => array(
						'key'        => 'layout_5b6c4aa24c122',
						'name'       => 'image',
						'label'      => __( 'Image', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5d6e6840e0423',
								'label'             => __( 'About this block', 'lesverts' ),
								'name'              => '',
								'aria-label'        => '',
								'type'              => 'message',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'message'           => __( 'Adds an image that scales automatically to the device size (responsive). This is the preferred method to add images.', 'lesverts' ),
								'new_lines'         => 'wpautop',
								'esc_html'          => 0,
							),
							array(
								'key'               => 'field_5b6c4aab4c123',
								'label'             => __( 'Image', 'lesverts' ),
								'name'              => 'image',
								'aria-label'        => '',
								'type'              => 'image',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'return_format'     => 'array',
								'preview_size'      => 'regular',
								'library'           => 'all',
								'min_width'         => '',
								'min_height'        => '',
								'min_size'          => '',
								'max_width'         => '',
								'max_height'        => '',
								'max_size'          => '',
								'mime_types'        => '',
							),
						),
						'min'        => '',
						'max'        => '',
					),
					'layout_5b6c4af94c126' => array(
						'key'        => 'layout_5b6c4af94c126',
						'name'       => 'oembed',
						'label'      => __( 'Video / Tweet / Post etc.', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b72a37b414b7',
								'label'             => __( 'About this Block', 'lesverts' ),
								'name'              => '',
								'aria-label'        => '',
								'type'              => 'message',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'message'           => __( 'Use this block to embed some posts, tweets, youtube videos etc. Just paste the link to the resource to embed and wait for the preview. If no preview appears, auto-embedding is not supported. You can then use a text block, switch to the text mode (on the upper right corner) and paste in the embed code provided from the platform that hosts the content.', 'lesverts' ),
								'new_lines'         => 'wpautop',
								'esc_html'          => 0,
							),
							array(
								'key'               => 'field_5b6c4b1b4c127',
								'label'             => __( 'Video / Tweet / Post etc.', 'lesverts' ),
								'name'              => 'oembed',
								'aria-label'        => '',
								'type'              => 'oembed',
								'instructions'      => __( 'Make sure you use the url to the tweet, post or video itself and not to the timeline.', 'lesverts' ),
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'width'             => 790,
								'height'            => 444,
							),
						),
						'min'        => '',
						'max'        => '',
					),
					'layout_5bbf722de1080' => array(
						'key'        => 'layout_5bbf722de1080',
						'name'       => 'person',
						'label'      => __( 'Person', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5bbf746e7edf9',
								'label'             => __( 'About this Block', 'lesverts' ),
								'name'              => '',
								'aria-label'        => '',
								'type'              => 'message',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'message'           => __( 'Use this block to present your members of parliament, your candidates etc.', 'lesverts' ),
								'new_lines'         => 'wpautop',
								'esc_html'          => 0,
							),
							array(
								'key'               => 'field_5bbf74e57edfa',
								'label'             => __( 'Person', 'lesverts' ),
								'name'              => 'person',
								'aria-label'        => '',
								'type'              => 'clone',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'clone'             => array(
									0 => 'group_5bbf71ef4ed64',
								),
								'display'           => 'seamless',
								'layout'            => 'block',
								'prefix_label'      => 0,
								'prefix_name'       => 0,
							),
						),
						'min'        => '',
						'max'        => '',
					),
					'layout_5bf563b4e8753' => array(
						'key'        => 'layout_5bf563b4e8753',
						'name'       => 'form',
						'label'      => __( 'Form', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5bf563b4e8754',
								'label'             => __( 'About this Block', 'lesverts' ),
								'name'              => '',
								'aria-label'        => '',
								'type'              => 'message',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'message'           => __( 'Show a form', 'lesverts' ),
								'new_lines'         => '',
								'esc_html'          => 0,
							),
							array(
								'key'               => 'field_5bf563b4e8755',
								'label'             => __( 'Form', 'lesverts' ),
								'name'              => 'form',
								'aria-label'        => '',
								'type'              => 'post_object',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'post_type'         => array(
									0 => 'theme_form',
								),
								'taxonomy'          => '',
								'allow_null'        => 0,
								'multiple'          => 0,
								'return_format'     => '',
								'ui'                => 1,
							),
						),
						'min'        => '',
						'max'        => '',
					),
					'layout_5ddec73648efd' => array(
						'key'        => 'layout_5ddec73648efd',
						'name'       => 'progress_bar',
						'label'      => __( 'Progress bar', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5ddec74348efe',
								'label'             => '',
								'name'              => '',
								'aria-label'        => '',
								'type'              => 'clone',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'clone'             => array(
									0 => 'group_5ddebe3fd4db3',
								),
								'display'           => 'seamless',
								'layout'            => 'block',
								'prefix_label'      => 0,
								'prefix_name'       => 0,
							),
						),
						'min'        => '',
						'max'        => '',
					),
					'layout_5c4f21cbd787c' => array(
						'key'        => 'layout_5c4f21cbd787c',
						'name'       => 'code',
						'label'      => __( 'Code', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5c4f21cbd787d',
								'label'             => __( 'About this Block', 'lesverts' ),
								'name'              => '',
								'aria-label'        => '',
								'type'              => 'message',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'message'           => __( 'Add custom code (advanced)', 'lesverts' ),
								'new_lines'         => '',
								'esc_html'          => 0,
							),
							array(
								'key'               => 'field_5c4f21cbd787d',
								'label'             => __( 'About this Block', 'lesverts' ),
								'name'              => '',
								'aria-label'        => '',
								'type'              => 'message',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'message'           => __( 'Add custom code (advanced)', 'lesverts' ),
								'new_lines'         => '',
								'esc_html'          => 0,
							),
							array(
								'key'               => 'field_5c4f21cbd787e',
								'label'             => __( 'Code', 'lesverts' ),
								'name'              => 'code',
								'aria-label'        => '',
								'type'              => 'acf_code_field',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'placeholder'       => '',
								'mode'              => 'htmlmixed',
								'theme'             => 'neo',
							),
							array(
								'key'               => 'field_5c4f21cbd787e',
								'label'             => __( 'Code', 'lesverts' ),
								'name'              => 'code',
								'aria-label'        => '',
								'type'              => 'acf_code_field',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'placeholder'       => '',
								'mode'              => 'htmlmixed',
								'theme'             => 'neo',
							),
						),
						'min'        => '',
						'max'        => '',
					),
					'layout_5d24b10e70f29' => array(
						'key'        => 'layout_5d24b10e70f29',
						'name'       => 'testimonials',
						'label'      => __( 'Testimonials', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5d24b11e70f2a',
								'label'             => '',
								'name'              => '',
								'aria-label'        => '',
								'type'              => 'clone',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'clone'             => array(
									0 => 'field_5b7da1211ced8',
									1 => 'field_5b6c608604fce',
								),
								'display'           => 'seamless',
								'layout'            => 'block',
								'prefix_label'      => 0,
								'prefix_name'       => 0,
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_5d6e67bae0421' => array(
						'key'        => 'layout_5d6e67bae0421',
						'name'       => 'block',
						'label'      => __( 'Block', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5fdbe4f88af4b',
								'label'             => '',
								'name'              => '',
								'aria-label'        => '',
								'type'              => 'clone',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'clone'             => array(
									0 => 'group_5fdbe45d039e4',
								),
								'display'           => 'seamless',
								'layout'            => 'block',
								'prefix_label'      => 0,
								'prefix_name'       => 0,
							),
						),
						'min'        => '',
						'max'        => '',
					),
					'layout_62062994bfc00' => array(
						'key'        => 'layout_62062994bfc00',
						'name'       => 'share_buttons',
						'label'      => __( 'Share Buttons', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_620629a0bfc01',
								'label'             => '',
								'name'              => '',
								'aria-label'        => '',
								'type'              => 'clone',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'clone'             => array(
									0 => 'group_620623a78f503',
								),
								'display'           => 'seamless',
								'layout'            => 'block',
								'prefix_label'      => 0,
								'prefix_name'       => 0,
							),
						),
						'min'        => '',
						'max'        => '',
					),
					'layout_635270c31365b' => array(
						'key'        => 'layout_635270c31365b',
						'name'       => 'gallery',
						'label'      => __( 'Gallery', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_635271081365d',
								'label'             => __( 'About this Block', 'lesverts' ),
								'name'              => '',
								'aria-label'        => '',
								'type'              => 'message',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'message'           => __( 'Photo gallery', 'lesverts' ),
								'new_lines'         => 'wpautop',
								'esc_html'          => 0,
							),
							array(
								'key'               => 'field_635270c81365c',
								'label'             => __( 'Images', 'lesverts' ),
								'name'              => 'images',
								'aria-label'        => '',
								'type'              => 'gallery',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'return_format'     => 'array',
								'library'           => 'all',
								'translations'      => 'copy_once',
								'min'               => '',
								'max'               => '',
								'min_width'         => '',
								'min_height'        => '',
								'min_size'          => '',
								'max_width'         => '',
								'max_height'        => '',
								'max_size'          => '',
								'mime_types'        => '',
								'insert'            => 'append',
								'preview_size'      => 'full-width',
							),
						),
						'min'        => '',
						'max'        => '',
					),
				),
				'min'               => '',
				'max'               => '',
				'button_label'      => __( 'Add Content Block', 'lesverts' ),
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'post',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => false,
		'description'           => '',
		'show_in_rest'          => 0,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b6ae2eb01de7',
		'title'                 => __( '[Template]Events Block', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5b7da28b9c989',
				'label'             => __( 'About this Block', 'lesverts' ),
				'name'              => '',
				'aria-label'        => '',
				'type'              => 'message',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'message'           => __( 'Displays a teaser of your most recent events.', 'lesverts' ),
				'new_lines'         => 'wpautop',
				'esc_html'          => 0,
			),
			array(
				'key'               => 'field_5b6ae7071a2b7',
				'label'             => __( 'Block Title', 'lesverts' ),
				'name'              => 'block_title',
				'aria-label'        => '',
				'type'              => 'clone',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'clone'             => array(
					0 => 'group_5b684e748247e',
				),
				'display'           => 'seamless',
				'layout'            => 'block',
				'prefix_label'      => 0,
				'prefix_name'       => 0,
			),
			array(
				'key'               => 'field_5b6ae32ac26d0',
				'label'             => __( 'Max events to show', 'lesverts' ),
				'name'              => 'max_num',
				'aria-label'        => '',
				'type'              => 'number',
				'instructions'      => __( 'Visitors can always see all events by clicking the «Show all events»-Button.', 'lesverts' ),
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'default_value'     => 4,
				'placeholder'       => '',
				'prepend'           => '',
				'append'            => '',
				'min'               => 2,
				'max'               => '',
				'step'              => 2,
			),
			array(
				'key'               => 'field_5fd768b29a6e7',
				'label'             => __( 'Event Category', 'lesverts' ),
				'name'              => 'show',
				'aria-label'        => '',
				'type'              => 'button_group',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'choices'           => array(
					'all'      => __( 'Show all upcoming events', 'lesverts' ),
					'category' => __( 'Only show events of specific category', 'lesverts' ),
				),
				'allow_null'        => 0,
				'default_value'     => 'all',
				'layout'            => 'horizontal',
				'return_format'     => 'value',
				'translations'      => 'ignore',
			),
			array(
				'key'               => 'field_5fd2a2ea57612',
				'label'             => __( 'Select Category', 'lesverts' ),
				'name'              => 'event_category',
				'aria-label'        => '',
				'type'              => 'taxonomy',
				'instructions'      => __( 'Select the category of events to display.', 'lesverts' ),
				'required'          => 0,
				'conditional_logic' => array(
					array(
						array(
							'field'    => 'field_5fd768b29a6e7',
							'operator' => '==',
							'value'    => 'category',
						),
					),
				),
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'taxonomy'          => 'tribe_events_cat',
				'field_type'        => 'select',
				'allow_null'        => 0,
				'add_term'          => 0,
				'save_terms'        => 0,
				'load_terms'        => 0,
				'return_format'     => 'id',
				'translations'      => 'copy_once',
				'multiple'          => 0,
			),
			array(
				'key'               => 'field_5b6ae4faa4259',
				'label'             => __( 'Full Width', 'lesverts' ),
				'name'              => 'full_width',
				'aria-label'        => '',
				'type'              => 'true_false',
				'instructions'      => __( 'If full width, the block will span the whole page (both columns) else it will only take the place of a single column block.', 'lesverts' ),
				'required'          => 0,
				'conditional_logic' => array(
					array(
						array(
							'field'    => 'field_5b6ae32ac26d0',
							'operator' => '>',
							'value'    => '2',
						),
						array(
							'field'    => 'field_5b6ae32ac26d0',
							'operator' => '<',
							'value'    => '6',
						),
					),
				),
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'message'           => '',
				'default_value'     => 1,
				'ui'                => 1,
				'ui_on_text'        => '',
				'ui_off_text'       => '',
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'page_type',
					'operator' => '==',
					'value'    => 'front_page',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => false,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1607954026,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b645b16b8a08',
		'title'                 => __( '[Template]Front Main Campaign', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5bfd36914df38',
				'label'             => __( 'Show Main Campaign', 'lesverts' ),
				'name'              => 'show_main_campaign',
				'aria-label'        => '',
				'type'              => 'true_false',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'message'           => __( 'Display primary campaign?', 'lesverts' ),
				'default_value'     => 1,
				'ui'                => 1,
				'ui_on_text'        => __( 'Show', 'lesverts' ),
				'ui_off_text'       => __( 'Hide', 'lesverts' ),
			),
			array(
				'key'               => 'field_5b645b8533eeb',
				'label'             => __( 'Image', 'lesverts' ),
				'name'              => 'image',
				'aria-label'        => '',
				'type'              => 'image',
				'instructions'      => __( 'Make sure the image is at least 2000px wide.', 'lesverts' ),
				'required'          => 1,
				'conditional_logic' => array(
					array(
						array(
							'field'    => 'field_5bfd36914df38',
							'operator' => '==',
							'value'    => '1',
						),
					),
				),
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'return_format'     => 'array',
				'preview_size'      => 'regular',
				'library'           => 'all',
				'min_width'         => 2000,
				'min_height'        => '',
				'min_size'          => '',
				'max_width'         => '',
				'max_height'        => '',
				'max_size'          => '',
				'mime_types'        => '',
			),
			array(
				'key'               => 'field_5b718424ad3be',
				'label'             => __( 'Bars', 'lesverts' ),
				'name'              => 'bars',
				'aria-label'        => '',
				'type'              => 'group',
				'instructions'      => __( 'The bars will be placed over the image in the corporate design.', 'lesverts' ),
				'required'          => 0,
				'conditional_logic' => array(
					array(
						array(
							'field'    => 'field_5bfd36914df38',
							'operator' => '==',
							'value'    => '1',
						),
					),
				),
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'layout'            => 'block',
				'sub_fields'        => array(
					array(
						'key'               => 'field_5b64b19806123',
						'label'             => __( 'Bar alignment', 'lesverts' ),
						'name'              => 'bar_alignment',
						'aria-label'        => '',
						'type'              => 'radio',
						'instructions'      => '',
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'choices'           => array(
							'left'  => __( 'Left', 'lesverts' ),
							'right' => __( 'Right', 'lesverts' ),
							'none'  => __( 'Do not show bars', 'lesverts' ),
						),
						'allow_null'        => 0,
						'other_choice'      => 0,
						'default_value'     => 'left : left',
						'layout'            => 'horizontal',
						'return_format'     => 'value',
						'save_other_choice' => 0,
					),
					array(
						'key'               => 'field_5b718193dfd9f',
						'label'             => __( 'Main Color', 'lesverts' ),
						'name'              => 'main_color',
						'aria-label'        => '',
						'type'              => 'button_group',
						'instructions'      => __( 'Select color of the upper headlines. Default is green. Use white, if the background is green already.', 'lesverts' ),
						'required'          => 0,
						'conditional_logic' => array(
							array(
								array(
									'field'    => 'field_5b64b19806123',
									'operator' => '!=',
									'value'    => 'none',
								),
							),
						),
						'wrapper'           => array(
							'width' => '30',
							'class' => '',
							'id'    => '',
						),
						'choices'           => array(
							'green' => __( 'Green', 'lesverts' ),
							'white' => __( 'White', 'lesverts' ),
						),
						'allow_null'        => 0,
						'default_value'     => 'green',
						'layout'            => 'vertical',
						'return_format'     => 'value',
					),
					array(
						'key'               => 'field_5b7184aaad3bf',
						'label'             => __( 'Headlines', 'lesverts' ),
						'name'              => 'headlines',
						'aria-label'        => '',
						'type'              => 'group',
						'instructions'      => '',
						'required'          => 0,
						'conditional_logic' => array(
							array(
								array(
									'field'    => 'field_5b64b19806123',
									'operator' => '!=',
									'value'    => 'none',
								),
							),
						),
						'wrapper'           => array(
							'width' => '70',
							'class' => '',
							'id'    => '',
						),
						'layout'            => 'block',
						'sub_fields'        => array(
							array(
								'key'               => 'field_5b645bd433eec',
								'label'             => '',
								'name'              => 'green',
								'aria-label'        => '',
								'type'              => 'repeater',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => array(
									array(
										array(
											'field'    => 'field_5b718193dfd9f',
											'operator' => '==',
											'value'    => 'green',
										),
									),
								),
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'collapsed'         => 'field_5b645c0833eed',
								'min'               => 1,
								'max'               => 2,
								'layout'            => '',
								'button_label'      => __( 'Add Green Headline', 'lesverts' ),
								'sub_fields'        => array(
									array(
										'key'               => 'field_5b645c0833eed',
										'label'             => __( 'Green Headline', 'lesverts' ),
										'name'              => 'bar',
										'aria-label'        => '',
										'type'              => 'text',
										'instructions'      => '',
										'required'          => 0,
										'conditional_logic' => 0,
										'wrapper'           => array(
											'width' => '',
											'class' => '',
											'id'    => '',
										),
										'default_value'     => '',
										'placeholder'       => '',
										'prepend'           => '',
										'append'            => '',
										'maxlength'         => 20,
										'parent_repeater'   => 'field_5b645bd433eec',
									),
								),
								'rows_per_page'     => 20,
							),
							array(
								'key'               => 'field_5b645c8433eee',
								'label'             => '',
								'name'              => 'white',
								'aria-label'        => '',
								'type'              => 'repeater',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => array(
									array(
										array(
											'field'    => 'field_5b718193dfd9f',
											'operator' => '==',
											'value'    => 'white',
										),
									),
								),
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'collapsed'         => 'field_5b645cf533eef',
								'min'               => 1,
								'max'               => 2,
								'layout'            => '',
								'button_label'      => __( 'Add White Headline', 'lesverts' ),
								'sub_fields'        => array(
									array(
										'key'               => 'field_5b645cf533eef',
										'label'             => __( 'White Headline', 'lesverts' ),
										'name'              => 'bar',
										'aria-label'        => '',
										'type'              => 'text',
										'instructions'      => '',
										'required'          => 0,
										'conditional_logic' => 0,
										'wrapper'           => array(
											'width' => '',
											'class' => '',
											'id'    => '',
										),
										'default_value'     => '',
										'placeholder'       => '',
										'prepend'           => '',
										'append'            => '',
										'maxlength'         => 20,
										'parent_repeater'   => 'field_5b645c8433eee',
									),
								),
								'rows_per_page'     => 20,
							),
							array(
								'key'               => 'field_5b645d3433ef0',
								'label'             => '',
								'name'              => 'magenta',
								'aria-label'        => '',
								'type'              => 'repeater',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'collapsed'         => 'field_5b645d6b33ef1',
								'min'               => 1,
								'max'               => 2,
								'layout'            => '',
								'button_label'      => __( 'Add Magenta Headline', 'lesverts' ),
								'sub_fields'        => array(
									array(
										'key'               => 'field_5b645d6b33ef1',
										'label'             => __( 'Magenta Headline', 'lesverts' ),
										'name'              => 'bar',
										'aria-label'        => '',
										'type'              => 'text',
										'instructions'      => '',
										'required'          => 0,
										'conditional_logic' => 0,
										'wrapper'           => array(
											'width' => '',
											'class' => '',
											'id'    => '',
										),
										'default_value'     => '',
										'placeholder'       => '',
										'prepend'           => '',
										'append'            => '',
										'maxlength'         => 20,
										'parent_repeater'   => 'field_5b645d3433ef0',
									),
								),
								'rows_per_page'     => 20,
							),
						),
					),
					array(
						'key'               => 'field_5b72aad898025',
						'label'             => __( 'Subline Color', 'lesverts' ),
						'name'              => 'subline_color',
						'aria-label'        => '',
						'type'              => 'button_group',
						'instructions'      => __( 'Select color of the sublines. Default is white. Use green, if the background is white already.', 'lesverts' ),
						'required'          => 0,
						'conditional_logic' => array(
							array(
								array(
									'field'    => 'field_5b64b19806123',
									'operator' => '!=',
									'value'    => 'none',
								),
							),
						),
						'wrapper'           => array(
							'width' => '30',
							'class' => '',
							'id'    => '',
						),
						'choices'           => array(
							'white' => __( 'White', 'lesverts' ),
							'green' => __( 'Green', 'lesverts' ),
						),
						'allow_null'        => 0,
						'default_value'     => 'white',
						'layout'            => 'vertical',
						'return_format'     => 'value',
					),
					array(
						'key'               => 'field_5b72aadc98026',
						'label'             => __( 'Sublines', 'lesverts' ),
						'name'              => 'sublines',
						'aria-label'        => '',
						'type'              => 'group',
						'instructions'      => '',
						'required'          => 0,
						'conditional_logic' => array(
							array(
								array(
									'field'    => 'field_5b64b19806123',
									'operator' => '!=',
									'value'    => 'none',
								),
							),
						),
						'wrapper'           => array(
							'width' => '70',
							'class' => '',
							'id'    => '',
						),
						'layout'            => 'block',
						'sub_fields'        => array(
							array(
								'key'               => 'field_5b72aadd9802d',
								'label'             => '',
								'name'              => 'green',
								'aria-label'        => '',
								'type'              => 'repeater',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => array(
									array(
										array(
											'field'    => 'field_5b72aad898025',
											'operator' => '==',
											'value'    => 'green',
										),
									),
								),
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'collapsed'         => 'field_5b72aadd9802e',
								'min'               => 0,
								'max'               => 2,
								'layout'            => 'table',
								'button_label'      => __( 'Add Subline', 'lesverts' ),
								'sub_fields'        => array(
									array(
										'key'               => 'field_5b72aadd9802e',
										'label'             => __( 'Green Subline', 'lesverts' ),
										'name'              => 'bar',
										'aria-label'        => '',
										'type'              => 'text',
										'instructions'      => '',
										'required'          => 0,
										'conditional_logic' => 0,
										'wrapper'           => array(
											'width' => '',
											'class' => '',
											'id'    => '',
										),
										'default_value'     => '',
										'placeholder'       => '',
										'prepend'           => '',
										'append'            => '',
										'maxlength'         => '',
										'parent_repeater'   => 'field_5b72aadd9802d',
									),
								),
								'rows_per_page'     => 20,
							),
							array(
								'key'               => 'field_5b72aadd9802f',
								'label'             => '',
								'name'              => 'white',
								'aria-label'        => '',
								'type'              => 'repeater',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => array(
									array(
										array(
											'field'    => 'field_5b72aad898025',
											'operator' => '==',
											'value'    => 'white',
										),
									),
								),
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'collapsed'         => 'field_5b72aadd98030',
								'min'               => 0,
								'max'               => 2,
								'layout'            => 'table',
								'button_label'      => __( 'Add Subline', 'lesverts' ),
								'sub_fields'        => array(
									array(
										'key'               => 'field_5b72aadd98030',
										'label'             => __( 'White Subline', 'lesverts' ),
										'name'              => 'bar',
										'aria-label'        => '',
										'type'              => 'text',
										'instructions'      => '',
										'required'          => 0,
										'conditional_logic' => 0,
										'wrapper'           => array(
											'width' => '',
											'class' => '',
											'id'    => '',
										),
										'default_value'     => '',
										'placeholder'       => '',
										'prepend'           => '',
										'append'            => '',
										'maxlength'         => '',
										'parent_repeater'   => 'field_5b72aadd9802f',
									),
								),
								'rows_per_page'     => 20,
							),
						),
					),
				),
			),
			array(
				'key'               => 'field_5b718e15c398d',
				'label'             => __( 'Campaign Summary', 'lesverts' ),
				'name'              => 'summary',
				'aria-label'        => '',
				'type'              => 'group',
				'instructions'      => __( 'This refers to the upper left box below the image. Add a brief description of the campaign.', 'lesverts' ),
				'required'          => 0,
				'conditional_logic' => array(
					array(
						array(
							'field'    => 'field_5bfd36914df38',
							'operator' => '==',
							'value'    => '1',
						),
					),
				),
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'layout'            => 'block',
				'sub_fields'        => array(
					array(
						'key'               => 'field_5b645e6b33ef7',
						'label'             => __( 'Title', 'lesverts' ),
						'name'              => 'title',
						'aria-label'        => '',
						'type'              => 'text',
						'instructions'      => __( 'This is the main title. Keep it as short as possible!', 'lesverts' ),
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'default_value'     => '',
						'placeholder'       => '',
						'prepend'           => '',
						'append'            => '',
						'maxlength'         => 80,
					),
					array(
						'key'               => 'field_5b645ec333ef8',
						'label'             => __( 'Teaser', 'lesverts' ),
						'name'              => 'teaser',
						'aria-label'        => '',
						'type'              => 'textarea',
						'instructions'      => __( 'Write a brief teaser to attract the readers interest. Keept it short (200 to 400 characters).', 'lesverts' ),
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'default_value'     => '',
						'placeholder'       => '',
						'maxlength'         => 500,
						'rows'              => 3,
						'new_lines'         => '',
					),
					array(
						'key'               => 'field_5b645edb33ef9',
						'label'             => __( 'Read more button', 'lesverts' ),
						'name'              => 'read_more',
						'aria-label'        => '',
						'type'              => 'link',
						'instructions'      => __( 'Link the main page of this campaign and set an accurate "read more" text for the link button.', 'lesverts' ),
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'return_format'     => 'array',
					),
				),
			),
			array(
				'key'               => 'field_5b718f74c398e',
				'label'             => __( 'Call to Action', 'lesverts' ),
				'name'              => 'call_to_action',
				'aria-label'        => '',
				'type'              => 'group',
				'instructions'      => __( 'This refers to the second box, the darker one. Use it to call for action.', 'lesverts' ),
				'required'          => 0,
				'conditional_logic' => array(
					array(
						array(
							'field'    => 'field_5bfd36914df38',
							'operator' => '==',
							'value'    => '1',
						),
					),
				),
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'layout'            => 'block',
				'sub_fields'        => array(
					array(
						'key'               => 'field_5b681c8590acf',
						'label'             => __( 'Title', 'lesverts' ),
						'name'              => 'title',
						'aria-label'        => '',
						'type'              => 'text',
						'instructions'      => __( 'Use an encouraging title, like \'Let me be part of the success\'.', 'lesverts' ),
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'default_value'     => '',
						'placeholder'       => '',
						'prepend'           => '',
						'append'            => '',
						'maxlength'         => '',
					),
					array(
						'key'               => 'field_5b681f7ccd058',
						'label'             => __( 'Description', 'lesverts' ),
						'name'              => 'description',
						'aria-label'        => '',
						'type'              => 'textarea',
						'instructions'      => __( 'Tell the visiter briefly, what the form does. Example: Stay updated, subscribe to our newsletter; make a change, become a member; sign the petition', 'lesverts' ),
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'default_value'     => '',
						'placeholder'       => '',
						'maxlength'         => '',
						'rows'              => '',
						'new_lines'         => '',
					),
					array(
						'key'               => 'field_5cf77bf9983f1',
						'label'             => __( 'Action Type', 'lesverts' ),
						'name'              => 'type',
						'aria-label'        => '',
						'type'              => 'button_group',
						'instructions'      => '',
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'choices'           => array(
							'form'   => __( 'Form', 'lesverts' ),
							'button' => __( 'Action Button', 'lesverts' ),
							'link'   => __( 'Link Button', 'lesverts' ),
						),
						'allow_null'        => 0,
						'default_value'     => 'form',
						'layout'            => 'horizontal',
						'return_format'     => 'value',
					),
					array(
						'key'               => 'field_5b681f2590ad0',
						'label'             => __( 'Call to Action (Form)', 'lesverts' ),
						'name'              => 'form',
						'aria-label'        => '',
						'type'              => 'post_object',
						'instructions'      => __( 'Displays directly the form with the call to action.', 'lesverts' ),
						'required'          => 0,
						'conditional_logic' => array(
							array(
								array(
									'field'    => 'field_5cf77bf9983f1',
									'operator' => '==',
									'value'    => 'form',
								),
							),
						),
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'post_type'         => array(
							0 => 'theme_form',
						),
						'taxonomy'          => '',
						'allow_null'        => 1,
						'multiple'          => 0,
						'return_format'     => 'id',
						'ui'                => 1,
					),
					array(
						'key'               => 'field_5cf77c8b983f2',
						'label'             => __( 'Call to Action (Button)', 'lesverts' ),
						'name'              => 'button',
						'aria-label'        => '',
						'type'              => 'link',
						'instructions'      => __( 'Displays the link to your action page as a magenta button.', 'lesverts' ),
						'required'          => 0,
						'conditional_logic' => array(
							array(
								array(
									'field'    => 'field_5cf77bf9983f1',
									'operator' => '==',
									'value'    => 'button',
								),
							),
						),
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'return_format'     => 'array',
					),
					array(
						'key'               => 'field_5cf77d16983f3',
						'label'             => __( 'Call to Action (Link Button)', 'lesverts' ),
						'name'              => 'link',
						'aria-label'        => '',
						'type'              => 'link',
						'instructions'      => __( 'Displays the link to further reading as link button (the green one with >).', 'lesverts' ),
						'required'          => 0,
						'conditional_logic' => array(
							array(
								array(
									'field'    => 'field_5cf77bf9983f1',
									'operator' => '==',
									'value'    => 'link',
								),
							),
						),
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'return_format'     => 'array',
					),
				),
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'post',
				),
			),
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'page',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => false,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1559732070,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b6c416ac235f',
		'title'                 => __( '[Template]In Short', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5b7293fda545d',
				'label'             => __( 'About this Block', 'lesverts' ),
				'name'              => '',
				'aria-label'        => '',
				'type'              => 'message',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'message'           => __( 'This is for visitors that don\'t want to read a lot.

Provide the key facts, preferably in bullet points and add the main call to action below.', 'lesverts' ),
				'new_lines'         => 'wpautop',
				'esc_html'          => 0,
			),
			array(
				'key'               => 'field_5b7293675fb5d',
				'label'             => __( 'Title', 'lesverts' ),
				'name'              => 'title',
				'aria-label'        => '',
				'type'              => 'text',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'default_value'     => 'The most important in brief',
				'placeholder'       => '',
				'prepend'           => '',
				'append'            => '',
				'maxlength'         => '',
			),
			array(
				'key'               => 'field_5b6c426cd8fbe',
				'label'             => __( 'Content', 'lesverts' ),
				'name'              => 'content',
				'aria-label'        => '',
				'type'              => 'flexible_content',
				'instructions'      => __( 'Add a brief description. Then add a call to action.', 'lesverts' ),
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'layouts'           => array(
					'layout_5b6c42dbd8fc1' => array(
						'key'        => 'layout_5b6c42dbd8fc1',
						'name'       => 'text',
						'label'      => __( 'Text', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b6c497c69760',
								'label'             => '',
								'name'              => 'text',
								'aria-label'        => '',
								'type'              => 'wysiwyg',
								'instructions'      => __( 'Use bullet points, if possible.', 'lesverts' ),
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'tabs'              => 'all',
								'toolbar'           => 'full',
								'media_upload'      => 1,
								'delay'             => 0,
							),
						),
						'min'        => '',
						'max'        => '',
					),
					'layout_5b6c498e69761' => array(
						'key'        => 'layout_5b6c498e69761',
						'name'       => 'form',
						'label'      => __( 'Form', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b6c499f69762',
								'label'             => __( 'Form', 'lesverts' ),
								'name'              => 'form',
								'aria-label'        => '',
								'type'              => 'post_object',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'post_type'         => array(
									0 => 'theme_form',
								),
								'taxonomy'          => '',
								'allow_null'        => 0,
								'multiple'          => 0,
								'return_format'     => 'id',
								'ui'                => 1,
							),
						),
						'min'        => '',
						'max'        => '',
					),
				),
				'button_label'      => __( 'Add In Short Content Block', 'lesverts' ),
				'min'               => 1,
				'max'               => '',
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'post',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => false,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1543420698,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b68487f04a16',
		'title'                 => __( '[Template]Latest Press Release', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5b7193201a0ac',
				'label'             => __( 'About this Block', 'lesverts' ),
				'name'              => '',
				'aria-label'        => '',
				'type'              => 'message',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'message'           => __( 'Display the latest press release (as long as it is published under the category you set below). Posts where the flag \'Show on front page\' is set to false will be excluded.

If the press release contains a quote block, the quote will be displayed. Else the lead is shown.', 'lesverts' ),
				'new_lines'         => 'wpautop',
				'esc_html'          => 0,
			),
			array(
				'key'               => 'field_5b68614337425',
				'label'             => __( 'Block Title', 'lesverts' ),
				'name'              => 'block_title',
				'aria-label'        => '',
				'type'              => 'clone',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'clone'             => array(
					0 => 'group_5b684e748247e',
				),
				'display'           => 'seamless',
				'layout'            => 'block',
				'prefix_label'      => 0,
				'prefix_name'       => 0,
			),
			array(
				'key'               => 'field_5b730cf648300',
				'label'             => __( 'Category', 'lesverts' ),
				'name'              => 'category',
				'aria-label'        => '',
				'type'              => 'taxonomy',
				'instructions'      => __( 'Select the category of your press releases.', 'lesverts' ),
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'taxonomy'          => 'category',
				'field_type'        => 'select',
				'allow_null'        => 0,
				'add_term'          => 0,
				'save_terms'        => 0,
				'load_terms'        => 0,
				'return_format'     => 'id',
				'multiple'          => 0,
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'page_type',
					'operator' => '==',
					'value'    => 'front_page',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => false,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1543324744,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b6c4bfaa8a26',
		'title'                 => __( '[Template]Link Lists', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5bfd14804bd11',
				'label'             => __( 'About this Block', 'lesverts' ),
				'name'              => '',
				'aria-label'        => '',
				'type'              => 'message',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'message'           => __( 'Use link list blocks to provide some additional information, the reader could be interested in.

A good first starting point would be a <i>manual link list</i> block that lists documents where the reader can dig deeper, like the debate in parliament etc. A second cool block <i>(auto link list)</i> could be the latest posts of the same category or with the same tag.', 'lesverts' ),
				'new_lines'         => 'wpautop',
				'esc_html'          => 0,
			),
			array(
				'key'               => 'field_5b7d1c098d538',
				'label'             => __( 'Title', 'lesverts' ),
				'name'              => 'title',
				'aria-label'        => '',
				'type'              => 'text',
				'instructions'      => __( 'Example: \'You might also be interested in\' or \' Related content\'', 'lesverts' ),
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'default_value'     => '',
				'placeholder'       => '',
				'prepend'           => '',
				'append'            => '',
				'maxlength'         => '',
			),
			array(
				'key'               => 'field_5b7c160fb4d15',
				'label'             => __( 'Link List Blocks', 'lesverts' ),
				'name'              => 'link_lists',
				'aria-label'        => '',
				'type'              => 'flexible_content',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'layouts'           => array(
					'5b7c16195e1e9'        => array(
						'key'        => '5b7c16195e1e9',
						'name'       => 'manual',
						'label'      => __( 'Manual Link List (Static)', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b7c20f594fb7',
								'label'             => __( 'About this Block', 'lesverts' ),
								'name'              => '',
								'aria-label'        => '',
								'type'              => 'message',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'message'           => __( 'Show a manually curated list of links. Links you add using this block will stay as you added them (static).', 'lesverts' ),
								'new_lines'         => 'wpautop',
								'esc_html'          => 0,
							),
							array(
								'key'               => 'field_5b7afba7b6b08',
								'label'             => __( 'Block Title', 'lesverts' ),
								'name'              => 'block_title',
								'aria-label'        => '',
								'type'              => 'clone',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'clone'             => array(
									0 => 'group_5b684e748247e',
								),
								'display'           => 'seamless',
								'layout'            => 'block',
								'prefix_label'      => 0,
								'prefix_name'       => 0,
							),
							array(
								'key'               => 'field_5b7af93309079',
								'label'             => __( 'Linked Content', 'lesverts' ),
								'name'              => 'linked_content',
								'aria-label'        => '',
								'type'              => 'flexible_content',
								'instructions'      => __( 'Add up to four links to posts, pages, events or external links.', 'lesverts' ),
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'layouts'           => array(
									'5b7af950bce91'        => array(
										'key'        => '5b7af950bce91',
										'name'       => 'links',
										'label'      => __( 'Link', 'lesverts' ),
										'display'    => 'row',
										'sub_fields' => array(
											array(
												'key'               => 'field_5b7af96e0907a',
												'label'             => __( 'Link', 'lesverts' ),
												'name'              => 'link',
												'aria-label'        => '',
												'type'              => 'link',
												'instructions'      => '',
												'required'          => 0,
												'conditional_logic' => 0,
												'wrapper'           => array(
													'width' => '',
													'class' => '',
													'id'    => '',
												),
												'return_format'     => 'array',
											),
										),
										'min'        => '',
										'max'        => '',
									),
									'layout_5b7afa685296a' => array(
										'key'        => 'layout_5b7afa685296a',
										'name'       => 'post_objects',
										'label'      => __( 'Single Post / Page / Event', 'lesverts' ),
										'display'    => 'row',
										'sub_fields' => array(
											array(
												'key'               => 'field_5b7af9910907b',
												'label'             => __( 'Post / Page / Event', 'lesverts' ),
												'name'              => 'post_object',
												'aria-label'        => '',
												'type'              => 'post_object',
												'instructions'      => '',
												'required'          => 0,
												'conditional_logic' => 0,
												'wrapper'           => array(
													'width' => '',
													'class' => '',
													'id'    => '',
												),
												'post_type'         => array(
													0 => 'post',
													1 => 'page',
													2 => 'tribe_events',
												),
												'taxonomy'          => array(),
												'allow_null'        => 0,
												'multiple'          => 0,
												'return_format'     => 'object',
												'ui'                => 1,
											),
										),
										'min'        => '',
										'max'        => '',
									),
								),
								'button_label'      => __( 'Add Link', 'lesverts' ),
								'min'               => 1,
								'max'               => 4,
							),
						),
						'min'        => '',
						'max'        => '',
					),
					'layout_5b7c165bb4d16' => array(
						'key'        => 'layout_5b7c165bb4d16',
						'name'       => 'auto',
						'label'      => __( 'Auto Link List (Dynamic)', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b7c253151b70',
								'label'             => __( 'Show Teaser', 'lesverts' ),
								'name'              => 'full_width',
								'aria-label'        => '',
								'type'              => 'true_false',
								'instructions'      => __( 'If true, the teaser of every content element is shown. This block will then span the whole content area (both columns).', 'lesverts' ),
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'message'           => '',
								'default_value'     => 0,
								'ui'                => 1,
								'ui_on_text'        => '',
								'ui_off_text'       => '',
							),
							array(
								'key'               => 'field_5b7c1af09b255',
								'label'             => __( 'Filter criteria', 'lesverts' ),
								'name'              => 'linked_content',
								'aria-label'        => '',
								'type'              => 'group',
								'instructions'      => __( 'The latest content that matches <strong>all</strong> of the selected categories and <strong>at least one</strong> of the selected tags will be displayed.<br><br>

If <strong>no tags</strong> are selected, content matching all of the categories will be displayed. If <strong>no categories</strong> are selected, all content matching any of the tags will be displayed.', 'lesverts' ),
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'layout'            => 'row',
								'sub_fields'        => array(
									array(
										'key'               => 'field_5b7c1b309b256',
										'label'             => __( 'Categories', 'lesverts' ),
										'name'              => 'categories',
										'aria-label'        => '',
										'type'              => 'taxonomy',
										'instructions'      => __( 'Content is displayed if <strong>all categories match</strong>.', 'lesverts' ),
										'required'          => 0,
										'conditional_logic' => 0,
										'wrapper'           => array(
											'width' => '',
											'class' => '',
											'id'    => '',
										),
										'taxonomy'          => 'category',
										'field_type'        => 'multi_select',
										'allow_null'        => 0,
										'add_term'          => 0,
										'save_terms'        => 0,
										'load_terms'        => 0,
										'return_format'     => 'id',
										'multiple'          => 0,
									),
									array(
										'key'               => 'field_5b7c1b539b257',
										'label'             => __( 'Tags', 'lesverts' ),
										'name'              => 'tags',
										'aria-label'        => '',
										'type'              => 'taxonomy',
										'instructions'      => __( 'Content is displayed if <strong>at lease one tag matches</strong>.', 'lesverts' ),
										'required'          => 0,
										'conditional_logic' => 0,
										'wrapper'           => array(
											'width' => '',
											'class' => '',
											'id'    => '',
										),
										'taxonomy'          => 'post_tag',
										'field_type'        => 'multi_select',
										'allow_null'        => 0,
										'add_term'          => 0,
										'save_terms'        => 0,
										'load_terms'        => 0,
										'return_format'     => 'id',
										'multiple'          => 0,
									),
								),
							),
							array(
								'key'               => 'field_5b7c21a694fb8',
								'label'             => __( 'About this Block', 'lesverts' ),
								'name'              => '',
								'aria-label'        => '',
								'type'              => 'message',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'message'           => __( 'Display the latest four posts, events or pages that meet the given filter criteria.

This list will automatically be updated if any new content meets the filter criteria (dynamic).', 'lesverts' ),
								'new_lines'         => 'wpautop',
								'esc_html'          => 0,
							),
							array(
								'key'               => 'field_5b7c16edb4d17',
								'label'             => __( 'Block Title', 'lesverts' ),
								'name'              => 'block_title',
								'aria-label'        => '',
								'type'              => 'clone',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'clone'             => array(
									0 => 'group_5b684e748247e',
								),
								'display'           => 'seamless',
								'layout'            => 'block',
								'prefix_label'      => 0,
								'prefix_name'       => 0,
							),
						),
						'min'        => '',
						'max'        => '',
					),
				),
				'button_label'      => __( 'Add Link List Block', 'lesverts' ),
				'min'               => '',
				'max'               => '',
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'post',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => false,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1543421049,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5d6e374bd3dcc',
		'title'                 => __( '[Template]Map', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5d6e37bf5918e',
				'label'             => __( 'About this Block – Advanced !', 'lesverts' ),
				'name'              => '',
				'aria-label'        => '',
				'type'              => 'message',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'message'           => __( 'This is an advanced block. You\'ll need to know the fundamentals of SVG to use it. It can be used to display a map of your region with a nice popup for every region if it is clicked. As well as an engaging text to become a member.', 'lesverts' ),
				'new_lines'         => '',
				'esc_html'          => 0,
			),
			array(
				'key'               => 'field_5d6e37df5918f',
				'label'             => __( 'Map SVG', 'lesverts' ),
				'name'              => 'map',
				'aria-label'        => '',
				'type'              => 'acf_code_field',
				'instructions'      => __( 'Copy the SVG in here. Make sure the regions have the \'.map-item\' class and the id matches the id of the data.', 'lesverts' ),
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'default_value'     => '',
				'placeholder'       => '',
				'mode'              => 'htmlmixed',
				'theme'             => 'neo',
			),
			array(
				'key'               => 'field_5d6e381759190',
				'label'             => __( 'Map Data', 'lesverts' ),
				'name'              => 'map_data',
				'aria-label'        => '',
				'type'              => 'repeater',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'collapsed'         => 'field_5d6e383459191',
				'min'               => 0,
				'max'               => 0,
				'layout'            => 'block',
				'button_label'      => 'Add Row',
				'sub_fields'        => array(
					array(
						'key'               => 'field_5d6e383459191',
						'label'             => __( 'Label', 'lesverts' ),
						'name'              => 'label',
						'aria-label'        => '',
						'type'              => 'text',
						'instructions'      => __( 'This is used in the dropdown of the mobile version.', 'lesverts' ),
						'required'          => 1,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'default_value'     => '',
						'placeholder'       => '',
						'prepend'           => '',
						'append'            => '',
						'maxlength'         => '',
						'parent_repeater'   => 'field_5d6e381759190',
					),
					array(
						'key'               => 'field_5d6e384b59192',
						'label'             => __( 'ID', 'lesverts' ),
						'name'              => 'value',
						'aria-label'        => '',
						'type'              => 'text',
						'instructions'      => __( 'This must match the id in the svg.', 'lesverts' ),
						'required'          => 1,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'default_value'     => '',
						'placeholder'       => '',
						'prepend'           => '',
						'append'            => '',
						'maxlength'         => '',
						'parent_repeater'   => 'field_5d6e381759190',
					),
					array(
						'key'               => 'field_5d6e386c59193',
						'label'             => __( 'Dialog Content', 'lesverts' ),
						'name'              => 'dialog_content',
						'aria-label'        => '',
						'type'              => 'repeater',
						'instructions'      => __( 'The content block will be separated by margin and a slight line.', 'lesverts' ),
						'required'          => 1,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'collapsed'         => '',
						'min'               => 1,
						'max'               => 0,
						'layout'            => 'row',
						'button_label'      => __( 'Add Content', 'lesverts' ),
						'sub_fields'        => array(
							array(
								'key'               => 'field_5d6e388759194',
								'label'             => __( 'Content', 'lesverts' ),
								'name'              => 'content',
								'aria-label'        => '',
								'type'              => 'wysiwyg',
								'instructions'      => '',
								'required'          => 1,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'tabs'              => 'all',
								'toolbar'           => 'full',
								'media_upload'      => 0,
								'delay'             => 0,
								'parent_repeater'   => 'field_5d6e386c59193',
							),
						),
						'rows_per_page'     => 20,
						'parent_repeater'   => 'field_5d6e381759190',
					),
				),
				'rows_per_page'     => 20,
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'post',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => false,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1569854763,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b6da383ba62c',
		'title'                 => __( '[Template]Map Block', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5d6e39d65a070',
				'label'             => '',
				'name'              => '',
				'aria-label'        => '',
				'type'              => 'clone',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'clone'             => array(
					0 => 'field_5d6e37bf5918e',
				),
				'display'           => 'seamless',
				'layout'            => 'block',
				'prefix_label'      => 0,
				'prefix_name'       => 0,
			),
			array(
				'key'               => 'field_5b6dae2576af9',
				'label'             => __( 'Block Title', 'lesverts' ),
				'name'              => 'block_title',
				'aria-label'        => '',
				'type'              => 'clone',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'clone'             => array(
					0 => 'group_5b684e748247e',
				),
				'display'           => 'seamless',
				'layout'            => 'block',
				'prefix_label'      => 0,
				'prefix_name'       => 0,
			),
			array(
				'key'               => 'field_5b6daeb276afb',
				'label'             => __( 'Title', 'lesverts' ),
				'name'              => 'title',
				'aria-label'        => '',
				'type'              => 'text',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'default_value'     => 'Get Active – with us, in your region!',
				'placeholder'       => '',
				'prepend'           => '',
				'append'            => '',
				'maxlength'         => 80,
			),
			array(
				'key'               => 'field_5b6daa7a89f92',
				'label'             => __( 'Text', 'lesverts' ),
				'name'              => 'text',
				'aria-label'        => '',
				'type'              => 'wysiwyg',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'default_value'     => '',
				'tabs'              => 'all',
				'toolbar'           => 'full',
				'media_upload'      => 1,
				'delay'             => 0,
			),
			array(
				'key'               => 'field_5b6db68857582',
				'label'             => __( 'Full Width (hidden)', 'lesverts' ),
				'name'              => 'full_width',
				'aria-label'        => '',
				'type'              => 'true_false',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => array(
					array(
						array(
							'field'    => 'field_5b6daeb276afb',
							'operator' => '!=empty',
						),
						array(
							'field'    => 'field_5b6daeb276afb',
							'operator' => '==empty',
						),
					),
				),
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'message'           => '',
				'default_value'     => 1,
				'ui'                => 0,
				'ui_on_text'        => '',
				'ui_off_text'       => '',
			),
			array(
				'key'               => 'field_5d6e399e5a06f',
				'label'             => __( 'Map Block', 'lesverts' ),
				'name'              => 'map_block',
				'aria-label'        => '',
				'type'              => 'post_object',
				'instructions'      => __( 'Select the block with the map.', 'lesverts' ),
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'post_type'         => array(
					0 => 'block',
				),
				'taxonomy'          => '',
				'allow_null'        => 0,
				'multiple'          => 0,
				'return_format'     => 'object',
				'ui'                => 1,
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'page_type',
					'operator' => '==',
					'value'    => 'front_page',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => false,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1567507149,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5bbf71ef4ed64',
		'title'                 => __( '[Template]Person', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5bbf725974660',
				'label'             => __( 'Person', 'lesverts' ),
				'name'              => 'person',
				'aria-label'        => '',
				'type'              => 'post_object',
				'instructions'      => __( 'Select a person. Its name, photo and contact details will be displayed.', 'lesverts' ),
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'post_type'         => array(
					0 => 'people',
				),
				'taxonomy'          => '',
				'allow_null'        => 0,
				'multiple'          => 0,
				'return_format'     => 'object',
				'ui'                => 1,
			),
			array(
				'key'               => 'field_5bbf732574661',
				'label'             => __( 'Names supplement', 'lesverts' ),
				'name'              => 'supplement',
				'aria-label'        => '',
				'type'              => 'text',
				'instructions'      => __( 'Displayed next to the name. Use it to mark the city, year of birth etc. or leave it blank.', 'lesverts' ),
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'default_value'     => '',
				'placeholder'       => '',
				'prepend'           => '',
				'append'            => '',
				'maxlength'         => 50,
			),
			array(
				'key'               => 'field_5bbf73b674662',
				'label'             => __( 'Role', 'lesverts' ),
				'name'              => 'role',
				'aria-label'        => '',
				'type'              => 'textarea',
				'instructions'      => __( 'Max 280 characters.', 'lesverts' ),
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'default_value'     => '',
				'placeholder'       => '',
				'maxlength'         => 280,
				'rows'              => 3,
				'new_lines'         => '',
			),
			array(
				'key'               => 'field_5bbf73d274663',
				'label'             => __( 'Additional Info', 'lesverts' ),
				'name'              => 'wysiwyg',
				'aria-label'        => '',
				'type'              => 'wysiwyg',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'default_value'     => '',
				'tabs'              => 'all',
				'toolbar'           => 'full',
				'media_upload'      => 1,
				'delay'             => 0,
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'post',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => false,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1543324747,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b6b14b2632f8',
		'title'                 => __( '[Template]Position', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5b6b14c533c22',
				'label'             => __( 'Position Block Title', 'lesverts' ),
				'name'              => 'title',
				'aria-label'        => '',
				'type'              => 'text',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'default_value'     => '',
				'placeholder'       => '',
				'prepend'           => '',
				'append'            => '',
				'maxlength'         => 80,
			),
			array(
				'key'               => 'field_5b6b151133c23',
				'label'             => __( 'Position', 'lesverts' ),
				'name'              => 'position',
				'aria-label'        => '',
				'type'              => 'repeater',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'collapsed'         => 'field_5b6b152f33c24',
				'min'               => 1,
				'max'               => 4,
				'layout'            => 'table',
				'button_label'      => __( 'Add Position', 'lesverts' ),
				'sub_fields'        => array(
					array(
						'key'               => 'field_5b6b152f33c24',
						'label'             => __( 'Position', 'lesverts' ),
						'name'              => 'position',
						'aria-label'        => '',
						'type'              => 'text',
						'instructions'      => __( 'Keep it very short, use just a few keywords. Max 120 characters.', 'lesverts' ),
						'required'          => 1,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'default_value'     => '',
						'placeholder'       => '',
						'prepend'           => '',
						'append'            => '',
						'maxlength'         => 120,
						'parent_repeater'   => 'field_5b6b151133c23',
					),
				),
				'rows_per_page'     => 20,
			),
			array(
				'key'               => 'field_5b6b24861e7f5',
				'label'             => __( 'Read More Button Target', 'lesverts' ),
				'name'              => 'read_more_target',
				'aria-label'        => '',
				'type'              => 'post_object',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'post_type'         => array(
					0 => 'post',
					1 => 'page',
				),
				'taxonomy'          => '',
				'allow_null'        => 1,
				'multiple'          => 0,
				'return_format'     => 'object',
				'ui'                => 1,
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'page_type',
					'operator' => '==',
					'value'    => 'front_page',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => false,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1558980124,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b6b162bb52ed',
		'title'                 => __( '[Template]Positions', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5b7da2d494e05',
				'label'             => __( 'About this Block', 'lesverts' ),
				'name'              => '',
				'aria-label'        => '',
				'type'              => 'message',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'message'           => __( 'Use this block to display a teaser of your political positions. Make sure you link a page, that will further detail your position.', 'lesverts' ),
				'new_lines'         => 'wpautop',
				'esc_html'          => 0,
			),
			array(
				'key'               => 'field_5b6b163dfeb4f',
				'label'             => __( 'Block Title', 'lesverts' ),
				'name'              => 'block_title',
				'aria-label'        => '',
				'type'              => 'clone',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'clone'             => array(
					0 => 'group_5b684e748247e',
				),
				'display'           => 'seamless',
				'layout'            => 'block',
				'prefix_label'      => 0,
				'prefix_name'       => 0,
			),
			array(
				'key'               => 'field_5b6b1670feb50',
				'label'             => __( 'Positions Group', 'lesverts' ),
				'name'              => 'positions',
				'aria-label'        => '',
				'type'              => 'repeater',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'collapsed'         => 'field_5b6b1687feb51',
				'min'               => 1,
				'max'               => 4,
				'layout'            => 'block',
				'button_label'      => __( 'Add Position Block', 'lesverts' ),
				'sub_fields'        => array(
					array(
						'key'               => 'field_5b6b1687feb51',
						'label'             => __( 'Position', 'lesverts' ),
						'name'              => 'position',
						'aria-label'        => '',
						'type'              => 'clone',
						'instructions'      => '',
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'clone'             => array(
							0 => 'group_5b6b14b2632f8',
						),
						'display'           => 'seamless',
						'layout'            => 'block',
						'prefix_label'      => 0,
						'prefix_name'       => 0,
						'parent_repeater'   => 'field_5b6b1670feb50',
					),
				),
				'rows_per_page'     => 20,
			),
			array(
				'key'               => 'field_5b6b2b49866f7',
				'label'             => __( 'Full Width', 'lesverts' ),
				'name'              => 'full_width',
				'aria-label'        => '',
				'type'              => 'true_false',
				'instructions'      => __( 'If full width is set to yes, the block will span the whole page (both columns) else it will only take the place of a single column block.', 'lesverts' ),
				'required'          => 0,
				'conditional_logic' => array(
					array(
						array(
							'field'    => 'field_5b6b1670feb50',
							'operator' => '>',
							'value'    => '2',
						),
						array(
							'field'    => 'field_5b6b1670feb50',
							'operator' => '<',
							'value'    => '6',
						),
					),
				),
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'message'           => '',
				'default_value'     => 1,
				'ui'                => 1,
				'ui_on_text'        => '',
				'ui_off_text'       => '',
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'page_type',
					'operator' => '==',
					'value'    => 'front_page',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => false,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1558979806,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b71a23bc757e',
		'title'                 => __( '[Template]Post selector', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5f5b53349d479',
				'label'             => __( 'Post Selection Mode', 'lesverts' ),
				'name'              => 'post_selection',
				'aria-label'        => '',
				'type'              => 'button_group',
				'instructions'      => __( 'Choose \'Manual\' to select the post or page that should be displayed. \'Automatic\' will display the latest post of the given category.', 'lesverts' ),
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'choices'           => array(
					'manual' => __( 'Manual', 'lesverts' ),
					'latest' => __( 'Automatic (latest)', 'lesverts' ),
				),
				'allow_null'        => 0,
				'default_value'     => 'manual',
				'layout'            => 'horizontal',
				'return_format'     => 'value',
				'translations'      => 'copy_once',
			),
			array(
				'key'               => 'field_5b71a24cf3711',
				'label'             => __( 'Post', 'lesverts' ),
				'name'              => 'post',
				'aria-label'        => '',
				'type'              => 'post_object',
				'instructions'      => __( 'Select the post to display. The teaser and the image of the post, will be displayed.', 'lesverts' ),
				'required'          => 1,
				'conditional_logic' => array(
					array(
						array(
							'field'    => 'field_5f5b53349d479',
							'operator' => '==',
							'value'    => 'manual',
						),
					),
				),
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'post_type'         => array(
					0 => 'post',
					1 => 'page',
				),
				'taxonomy'          => '',
				'allow_null'        => 0,
				'multiple'          => 0,
				'return_format'     => 'object',
				'translations'      => 'copy_once',
				'ui'                => 1,
			),
			array(
				'key'               => 'field_5f5b54b49d47b',
				'label'             => __( 'Category', 'lesverts' ),
				'name'              => 'category',
				'aria-label'        => '',
				'type'              => 'taxonomy',
				'instructions'      => __( 'Select the category of which the lasted post should be displayed. The teaser and the image of the post, will be shown.', 'lesverts' ),
				'required'          => 1,
				'conditional_logic' => array(
					array(
						array(
							'field'    => 'field_5f5b53349d479',
							'operator' => '==',
							'value'    => 'latest',
						),
					),
				),
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'taxonomy'          => 'category',
				'field_type'        => 'select',
				'allow_null'        => 0,
				'add_term'          => 0,
				'save_terms'        => 0,
				'load_terms'        => 0,
				'return_format'     => 'id',
				'translations'      => 'copy_once',
				'multiple'          => 0,
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'post',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => false,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1599839070,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5ddebe3fd4db3',
		'title'                 => __( '[Template]Progress Bar', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5ddebe5f8a22d',
				'label'             => __( 'About this Block', 'lesverts' ),
				'name'              => '',
				'aria-label'        => '',
				'type'              => 'message',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'message'           => __( 'Shows a progress meter. This is particularly useful for petitions.', 'lesverts' ),
				'new_lines'         => '',
				'esc_html'          => 0,
			),
			array(
				'key'               => 'field_5ddebf318a22e',
				'label'             => __( 'Datasource', 'lesverts' ),
				'name'              => 'datasource',
				'aria-label'        => '',
				'type'              => 'button_group',
				'instructions'      => __( 'If form is selected, the number of form submissions will be displayed. Enter manually let\'s you specify the number displayed in a input field here.', 'lesverts' ),
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'choices'           => array(
					'form'   => __( 'Form', 'lesverts' ),
					'manual' => __( 'Enter manually', 'lesverts' ),
				),
				'allow_null'        => 0,
				'default_value'     => 'form',
				'layout'            => 'horizontal',
				'return_format'     => 'value',
			),
			array(
				'key'               => 'field_5ddec02e8a22f',
				'label'             => __( 'Form', 'lesverts' ),
				'name'              => 'form',
				'aria-label'        => '',
				'type'              => 'post_object',
				'instructions'      => __( 'The progress meter will display the number of submissions.', 'lesverts' ),
				'required'          => 1,
				'conditional_logic' => array(
					array(
						array(
							'field'    => 'field_5ddebf318a22e',
							'operator' => '==',
							'value'    => 'form',
						),
					),
				),
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'post_type'         => array(
					0 => 'theme_form',
				),
				'taxonomy'          => '',
				'allow_null'        => 0,
				'multiple'          => 0,
				'return_format'     => 'id',
				'ui'                => 1,
			),
			array(
				'key'               => 'field_5ddec0aa8a230',
				'label'             => __( 'Submissions to add', 'lesverts' ),
				'name'              => 'offset',
				'aria-label'        => '',
				'type'              => 'number',
				'instructions'      => __( 'Sometimes you do also have another datasource you want to add or you just don\'t want to start with zero. <strong>Specify the number you want to add to the total of submissions.</strong>', 'lesverts' ),
				'required'          => 1,
				'conditional_logic' => array(
					array(
						array(
							'field'    => 'field_5ddebf318a22e',
							'operator' => '==',
							'value'    => 'form',
						),
					),
				),
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'default_value'     => 0,
				'placeholder'       => '',
				'prepend'           => '',
				'append'            => '',
				'min'               => 0,
				'max'               => '',
				'step'              => '',
			),
			array(
				'key'               => 'field_5ddec1a08a231',
				'label'             => __( 'Current progress', 'lesverts' ),
				'name'              => 'current',
				'aria-label'        => '',
				'type'              => 'number',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => array(
					array(
						array(
							'field'    => 'field_5ddebf318a22e',
							'operator' => '==',
							'value'    => 'manual',
						),
					),
				),
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'default_value'     => '',
				'placeholder'       => '',
				'prepend'           => '',
				'append'            => '',
				'min'               => 0,
				'max'               => '',
				'step'              => '',
			),
			array(
				'key'               => 'field_5ddec1fe8a232',
				'label'             => __( 'Goal', 'lesverts' ),
				'name'              => 'goal',
				'aria-label'        => '',
				'type'              => 'button_group',
				'instructions'      => __( 'Specifies the maximum value (at which value the progress bar reaches the end). You may either set the goal manually or, if you choose adaptive, the goal will increase automatically with the current progress. The algorithm chooses the goal so that the progress bar reaches the right halve and the goal is a nice number (however, the goal never goes below ten).', 'lesverts' ),
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'choices'           => array(
					'adaptive' => __( 'Adaptive', 'lesverts' ),
					'manual'   => __( 'Manual', 'lesverts' ),
				),
				'allow_null'        => 0,
				'default_value'     => 'adaptive',
				'layout'            => 'horizontal',
				'return_format'     => 'value',
			),
			array(
				'key'               => 'field_5ddec29a8a233',
				'label'             => __( 'Goal Value', 'lesverts' ),
				'name'              => 'goal_value',
				'aria-label'        => '',
				'type'              => 'number',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => array(
					array(
						array(
							'field'    => 'field_5ddec1fe8a232',
							'operator' => '==',
							'value'    => 'manual',
						),
					),
				),
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'default_value'     => '',
				'placeholder'       => '',
				'prepend'           => '',
				'append'            => '',
				'min'               => 0,
				'max'               => '',
				'step'              => '',
			),
			array(
				'key'               => 'field_5ddec4478a234',
				'label'             => __( 'Legend (optional)', 'lesverts' ),
				'name'              => 'legend',
				'aria-label'        => '',
				'type'              => 'text',
				'instructions'      => __( 'If specified, a legend is shown above the progress bar. You can use the following placeholders: <code>{{current}}</code>, <code>{{goal}}</code>

Example: {{current}} have signed. Can we reach {{goal}}?', 'lesverts' ),
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'default_value'     => '',
				'placeholder'       => '',
				'prepend'           => '',
				'append'            => '',
				'maxlength'         => '',
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'post',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => false,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1574881022,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b6c045f68e96',
		'title'                 => __( '[Template]Quote', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5b72a5ba9a5e6',
				'label'             => __( 'About this Block', 'lesverts' ),
				'name'              => '',
				'aria-label'        => '',
				'type'              => 'message',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'message'           => __( 'Displays a nicely formatted quote of any person you added in your people section. The image will automatically be pulled from the persons record.', 'lesverts' ),
				'new_lines'         => 'wpautop',
				'esc_html'          => 0,
			),
			array(
				'key'               => 'field_5b6c0475af343',
				'label'             => __( 'Quote', 'lesverts' ),
				'name'              => 'quote',
				'aria-label'        => '',
				'type'              => 'textarea',
				'instructions'      => __( 'Max 280 character.', 'lesverts' ),
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'default_value'     => '',
				'placeholder'       => '',
				'maxlength'         => '',
				'rows'              => 4,
				'new_lines'         => '',
			),
			array(
				'key'               => 'field_5b6c049caf344',
				'label'             => __( 'Person', 'lesverts' ),
				'name'              => 'person',
				'aria-label'        => '',
				'type'              => 'post_object',
				'instructions'      => __( 'Select a person defined in the \'Peoples\' menu. The name and the image of this person will be used to display the quote.', 'lesverts' ),
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'post_type'         => array(
					0 => 'people',
				),
				'taxonomy'          => array(),
				'allow_null'        => 0,
				'multiple'          => 0,
				'return_format'     => 'object',
				'ui'                => 1,
			),
			array(
				'key'               => 'field_5b6c0b8cb34c3',
				'label'             => __( 'Role', 'lesverts' ),
				'name'              => 'role',
				'aria-label'        => '',
				'type'              => 'text',
				'instructions'      => __( 'Example: President of the Greens Uri', 'lesverts' ),
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'default_value'     => '',
				'placeholder'       => '',
				'prepend'           => '',
				'append'            => '',
				'maxlength'         => '',
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'post',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => false,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1547039495,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b6846d09aa72',
		'title'                 => __( '[Template]Single Post', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5b7d9f270b51d',
				'label'             => __( 'About this Block', 'lesverts' ),
				'name'              => '',
				'aria-label'        => '',
				'type'              => 'message',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'message'           => __( 'Display a teaser of any post or page.', 'lesverts' ),
				'new_lines'         => 'wpautop',
				'esc_html'          => 0,
			),
			array(
				'key'               => 'field_5b6861bb4db05',
				'label'             => __( 'Block Title', 'lesverts' ),
				'name'              => 'block_title',
				'aria-label'        => '',
				'type'              => 'clone',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'clone'             => array(
					0 => 'group_5b684e748247e',
				),
				'display'           => 'seamless',
				'layout'            => 'block',
				'prefix_label'      => 0,
				'prefix_name'       => 0,
			),
			array(
				'key'               => 'field_5b6846f16fb72',
				'label'             => __( 'Post', 'lesverts' ),
				'name'              => 'post',
				'aria-label'        => '',
				'type'              => 'clone',
				'instructions'      => __( 'Select the post to display. The excerpt and the image of the post, will be displayed.', 'lesverts' ),
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'clone'             => array(
					0 => 'group_5b71a23bc757e',
				),
				'display'           => 'seamless',
				'layout'            => 'block',
				'prefix_label'      => 0,
				'prefix_name'       => 0,
			),
			array(
				'key'               => 'field_5b719f7e7fcc0',
				'label'             => __( 'Footer', 'lesverts' ),
				'name'              => 'footer',
				'aria-label'        => '',
				'type'              => 'group',
				'instructions'      => __( 'Immediate call to action.', 'lesverts' ),
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'layout'            => 'row',
				'sub_fields'        => array(
					array(
						'key'               => 'field_5b6847db6fb75',
						'label'             => __( 'Call to Action Link', 'lesverts' ),
						'name'              => 'cta_link',
						'aria-label'        => '',
						'type'              => 'url',
						'instructions'      => __( 'Add anchor link to the call to action form.', 'lesverts' ),
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'default_value'     => '',
						'placeholder'       => '',
					),
					array(
						'key'               => 'field_5b68476a6fb74',
						'label'             => __( 'Call to Action Button Text', 'lesverts' ),
						'name'              => 'cta_label',
						'aria-label'        => '',
						'type'              => 'text',
						'instructions'      => '',
						'required'          => 1,
						'conditional_logic' => array(
							array(
								array(
									'field'    => 'field_5b6847db6fb75',
									'operator' => '!=empty',
								),
							),
						),
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'default_value'     => 'Get active',
						'placeholder'       => '',
						'prepend'           => '',
						'append'            => '',
						'maxlength'         => '',
					),
					array(
						'key'               => 'field_5b6847346fb73',
						'label'             => __( 'Call to Action Description', 'lesverts' ),
						'name'              => 'cta_desc',
						'aria-label'        => '',
						'type'              => 'text',
						'instructions'      => __( 'One very short sentence that explains why someone should click the call to action. Example: I want to get active against GMO.', 'lesverts' ),
						'required'          => 1,
						'conditional_logic' => array(
							array(
								array(
									'field'    => 'field_5b6847db6fb75',
									'operator' => '!=empty',
								),
							),
						),
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'default_value'     => '',
						'placeholder'       => '',
						'prepend'           => '',
						'append'            => '',
						'maxlength'         => '',
					),
				),
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'page_type',
					'operator' => '==',
					'value'    => 'front_page',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => false,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1543324749,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5bcee6dd2cfd7',
		'title'                 => __( '[Template]Teaser', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5bcee7167c5c5',
				'label'             => __( 'Teaser', 'lesverts' ),
				'name'              => 'teaser',
				'aria-label'        => '',
				'type'              => 'textarea',
				'instructions'      => __( '<strong>The first 280 characters</strong> will be used as preview text on the front page, archive pages and for search results.', 'lesverts' ),
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'default_value'     => '',
				'placeholder'       => 'Brief summary that attracts the readers interest. Make sure it contains the most important keywords.',
				'maxlength'         => '',
				'rows'              => 5,
				'new_lines'         => '',
			),
			array(
				'key'               => 'field_5bcee79ac84ae',
				'label'             => __( 'Use for preview only (hide on page)', 'lesverts' ),
				'name'              => 'teaser_hide_on_page',
				'aria-label'        => '',
				'type'              => 'true_false',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'message'           => __( 'If yes, the teaser will only be shown as preview text but not on the page itself.', 'lesverts' ),
				'default_value'     => 1,
				'ui'                => 1,
				'ui_on_text'        => '',
				'ui_off_text'       => '',
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'post',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => false,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1543324750,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b6c6072e2d3f',
		'title'                 => __( '[Template]Testimonials', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5b7da1211ced8',
				'label'             => __( 'About this Block', 'lesverts' ),
				'name'              => '',
				'aria-label'        => '',
				'type'              => 'message',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'message'           => __( 'Display some fancy testimonials. Make sure you add the testimonials with the corresponding <strong>category</strong> in the peoples section.', 'lesverts' ),
				'new_lines'         => 'wpautop',
				'esc_html'          => 0,
			),
			array(
				'key'               => 'field_5b6c6e387d12b',
				'label'             => __( 'Block Title', 'lesverts' ),
				'name'              => 'block_title',
				'aria-label'        => '',
				'type'              => 'clone',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'clone'             => array(
					0 => 'group_5b684e748247e',
				),
				'display'           => 'seamless',
				'layout'            => 'block',
				'prefix_label'      => 0,
				'prefix_name'       => 0,
			),
			array(
				'key'               => 'field_5bbb4e32cc149',
				'label'             => __( 'Title', 'lesverts' ),
				'name'              => 'title',
				'aria-label'        => '',
				'type'              => 'text',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'default_value'     => '',
				'placeholder'       => '',
				'prepend'           => '',
				'append'            => '',
				'maxlength'         => 140,
			),
			array(
				'key'               => 'field_5b6c608604fce',
				'label'             => __( 'Testimonials', 'lesverts' ),
				'name'              => 'taxonomy',
				'aria-label'        => '',
				'type'              => 'taxonomy',
				'instructions'      => __( 'Select the testimonials to display by its category.', 'lesverts' ),
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'taxonomy'          => 'testimonials',
				'field_type'        => 'multi_select',
				'allow_null'        => 0,
				'add_term'          => 0,
				'save_terms'        => 0,
				'load_terms'        => 0,
				'return_format'     => 'id',
				'multiple'          => 0,
			),
			array(
				'key'               => 'field_5ddd2e538374c',
				'label'             => __( 'Layout (hidden)', 'lesverts' ),
				'name'              => 'layout',
				'aria-label'        => '',
				'type'              => 'text',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => array(
					array(
						array(
							'field'    => 'field_5bbb4e32cc149',
							'operator' => '!=empty',
						),
						array(
							'field'    => 'field_5bbb4e32cc149',
							'operator' => '==empty',
						),
					),
				),
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'default_value'     => 'horizontal',
				'placeholder'       => '',
				'prepend'           => '',
				'append'            => '',
				'maxlength'         => '',
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'post',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => false,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1574776459,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b684a4bdffb6',
		'title'                 => __( '[Template]Two Posts', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5b7d9fb145d30',
				'label'             => __( 'About this Block', 'lesverts' ),
				'name'              => '',
				'aria-label'        => '',
				'type'              => 'message',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'message'           => __( 'Displays two teasers of any post or page.', 'lesverts' ),
				'new_lines'         => 'wpautop',
				'esc_html'          => 0,
			),
			array(
				'key'               => 'field_5b6861e519396',
				'label'             => __( 'Block Title', 'lesverts' ),
				'name'              => 'block_title',
				'aria-label'        => '',
				'type'              => 'clone',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'clone'             => array(
					0 => 'group_5b684e748247e',
				),
				'display'           => 'seamless',
				'layout'            => 'block',
				'prefix_label'      => 0,
				'prefix_name'       => 0,
			),
			array(
				'key'               => 'field_5b684a4be8d5e',
				'label'             => __( 'Post 1', 'lesverts' ),
				'name'              => 'post_1',
				'aria-label'        => '',
				'type'              => 'clone',
				'instructions'      => __( 'Select the post to display. The excerpt and the image of the post, will be displayed.', 'lesverts' ),
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'clone'             => array(
					0 => 'group_5b71a23bc757e',
				),
				'display'           => 'group',
				'layout'            => 'block',
				'prefix_label'      => 0,
				'prefix_name'       => 1,
			),
			array(
				'key'               => 'field_5b684a644d73a',
				'label'             => __( 'Post 2', 'lesverts' ),
				'name'              => 'post_2',
				'aria-label'        => '',
				'type'              => 'clone',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'clone'             => array(
					0 => 'group_5b71a23bc757e',
				),
				'display'           => 'group',
				'layout'            => 'block',
				'prefix_label'      => 0,
				'prefix_name'       => 1,
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'page_type',
					'operator' => '==',
					'value'    => 'front_page',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => false,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1599821566,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b6841bfef6ab',
		'title'                 => __( '[Template]Vote Recommendations', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5b7da085dc6f1',
				'label'             => __( 'About this Block', 'lesverts' ),
				'name'              => '',
				'aria-label'        => '',
				'type'              => 'message',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'message'           => __( 'Displays voting recommendations.', 'lesverts' ),
				'new_lines'         => 'wpautop',
				'esc_html'          => 0,
			),
			array(
				'key'               => 'field_5b68621965af5',
				'label'             => __( 'Category', 'lesverts' ),
				'name'              => 'category',
				'aria-label'        => '',
				'type'              => 'clone',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'clone'             => array(
					0 => 'group_5b684e748247e',
				),
				'display'           => 'seamless',
				'layout'            => 'block',
				'prefix_label'      => 0,
				'prefix_name'       => 0,
			),
			array(
				'key'               => 'field_5b68447d19735',
				'label'             => __( 'Recommendations', 'lesverts' ),
				'name'              => 'proposals',
				'aria-label'        => '',
				'type'              => 'repeater',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'collapsed'         => '',
				'min'               => 1,
				'max'               => 0,
				'layout'            => 'table',
				'button_label'      => __( 'Add recommendation', 'lesverts' ),
				'sub_fields'        => array(
					array(
						'key'               => 'field_5baa445a76610',
						'label'             => __( 'Recommendation Link', 'lesverts' ),
						'name'              => 'proposal_link',
						'aria-label'        => '',
						'type'              => 'group',
						'instructions'      => __( 'Where visitors can read more about this bill.', 'lesverts' ),
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '70',
							'class' => '',
							'id'    => '',
						),
						'layout'            => 'block',
						'sub_fields'        => array(
							array(
								'key'               => 'field_5baa448d76611',
								'label'             => __( 'Link Type', 'lesverts' ),
								'name'              => 'type',
								'aria-label'        => '',
								'type'              => 'button_group',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '25',
									'class' => '',
									'id'    => '',
								),
								'choices'           => array(
									'internal' => __( 'Internal', 'lesverts' ),
									'url'      => __( 'External (URL)', 'lesverts' ),
								),
								'allow_null'        => 0,
								'default_value'     => 'internal',
								'layout'            => 'vertical',
								'return_format'     => 'value',
							),
							array(
								'key'               => 'field_5c46226447063',
								'label'             => __( 'Internal Link', 'lesverts' ),
								'name'              => 'internal_link',
								'aria-label'        => '',
								'type'              => 'group',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => array(
									array(
										array(
											'field'    => 'field_5baa448d76611',
											'operator' => '==',
											'value'    => 'internal',
										),
									),
								),
								'wrapper'           => array(
									'width' => '75',
									'class' => '',
									'id'    => '',
								),
								'layout'            => 'block',
								'sub_fields'        => array(
									array(
										'key'               => 'field_5baa451976612',
										'label'             => __( 'Post', 'lesverts' ),
										'name'              => 'post',
										'aria-label'        => '',
										'type'              => 'post_object',
										'instructions'      => '',
										'required'          => 0,
										'conditional_logic' => 0,
										'wrapper'           => array(
											'width' => '',
											'class' => '',
											'id'    => '',
										),
										'post_type'         => array(
											0 => 'post',
											1 => 'page',
										),
										'taxonomy'          => '',
										'allow_null'        => 0,
										'multiple'          => 0,
										'return_format'     => 'object',
										'ui'                => 1,
									),
									array(
										'key'               => 'field_5c4620e993f91',
										'label'             => __( 'Title Overwrite (optional)', 'lesverts' ),
										'name'              => 'title',
										'aria-label'        => '',
										'type'              => 'text',
										'instructions'      => __( 'Leave blank, to use the posts title.', 'lesverts' ),
										'required'          => 0,
										'conditional_logic' => 0,
										'wrapper'           => array(
											'width' => '',
											'class' => '',
											'id'    => '',
										),
										'default_value'     => '',
										'placeholder'       => '',
										'prepend'           => '',
										'append'            => '',
										'maxlength'         => '',
									),
								),
							),
							array(
								'key'               => 'field_5baa456476613',
								'label'             => __( 'External Link (URL)', 'lesverts' ),
								'name'              => 'external',
								'aria-label'        => '',
								'type'              => 'group',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => array(
									array(
										array(
											'field'    => 'field_5baa448d76611',
											'operator' => '==',
											'value'    => 'url',
										),
									),
								),
								'wrapper'           => array(
									'width' => '75',
									'class' => '',
									'id'    => '',
								),
								'layout'            => 'block',
								'sub_fields'        => array(
									array(
										'key'               => 'field_5baa459776614',
										'label'             => __( 'URL', 'lesverts' ),
										'name'              => 'url',
										'aria-label'        => '',
										'type'              => 'url',
										'instructions'      => '',
										'required'          => 0,
										'conditional_logic' => 0,
										'wrapper'           => array(
											'width' => '',
											'class' => '',
											'id'    => '',
										),
										'default_value'     => '',
										'placeholder'       => '',
									),
									array(
										'key'               => 'field_5baa45ac76615',
										'label'             => __( 'Title', 'lesverts' ),
										'name'              => 'title',
										'aria-label'        => '',
										'type'              => 'text',
										'instructions'      => '',
										'required'          => 0,
										'conditional_logic' => 0,
										'wrapper'           => array(
											'width' => '',
											'class' => '',
											'id'    => '',
										),
										'default_value'     => '',
										'placeholder'       => '',
										'prepend'           => '',
										'append'            => '',
										'maxlength'         => '',
									),
									array(
										'key'               => 'field_5baa45b976616',
										'label'             => __( 'Teaser', 'lesverts' ),
										'name'              => 'teaser',
										'aria-label'        => '',
										'type'              => 'textarea',
										'instructions'      => '',
										'required'          => 0,
										'conditional_logic' => 0,
										'wrapper'           => array(
											'width' => '',
											'class' => '',
											'id'    => '',
										),
										'default_value'     => '',
										'placeholder'       => '',
										'maxlength'         => 280,
										'rows'              => 5,
										'new_lines'         => '',
									),
								),
							),
						),
						'parent_repeater'   => 'field_5b68447d19735',
					),
					array(
						'key'               => 'field_5b6844e919737',
						'label'             => __( 'Recommendation', 'lesverts' ),
						'name'              => 'flag',
						'aria-label'        => '',
						'type'              => 'radio',
						'instructions'      => __( 'What do you recommend to vote?', 'lesverts' ),
						'required'          => 1,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '30',
							'class' => '',
							'id'    => '',
						),
						'choices'           => array(
							'yes'             => __( 'Yes', 'lesverts' ),
							'no'              => __( 'No', 'lesverts' ),
							'free'            => __( '&mdash;', 'lesverts' ),
							'free2'           => __( 'Free Vote', 'lesverts' ),
							'blank'           => __( 'Blank Vote', 'lesverts' ),
							'initiative'      => __( 'Initiative', 'lesverts' ),
							'counter-project' => __( 'Counter Project', 'lesverts' ),
						),
						'default_value'     => '',
						'return_format'     => 'array',
						'translations'      => 'copy_once',
						'allow_null'        => 0,
						'other_choice'      => 0,
						'layout'            => 'vertical',
						'save_other_choice' => 0,
						'parent_repeater'   => 'field_5b68447d19735',
					),
				),
				'rows_per_page'     => 20,
			),
			array(
				'key'               => 'field_5b719d2713c9f',
				'label'             => __( 'Footer', 'lesverts' ),
				'name'              => 'footer',
				'aria-label'        => '',
				'type'              => 'group',
				'instructions'      => __( 'Only displayed, if link is set.', 'lesverts' ),
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'layout'            => 'row',
				'sub_fields'        => array(
					array(
						'key'               => 'field_5b687a7954f7d',
						'label'             => __( 'All Votations Link', 'lesverts' ),
						'name'              => 'link',
						'aria-label'        => '',
						'type'              => 'page_link',
						'instructions'      => __( 'Page with link to all vote recommendations (national, cantonal, community).', 'lesverts' ),
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'post_type'         => array(
							0 => 'page',
							1 => 'post',
						),
						'taxonomy'          => '',
						'allow_null'        => 0,
						'allow_archives'    => 0,
						'multiple'          => 0,
					),
					array(
						'key'               => 'field_5b687a2c54f7c',
						'label'             => __( 'Link Text', 'lesverts' ),
						'name'              => 'link_label',
						'aria-label'        => '',
						'type'              => 'text',
						'instructions'      => '',
						'required'          => 0,
						'conditional_logic' => array(
							array(
								array(
									'field'    => 'field_5b687a7954f7d',
									'operator' => '!=empty',
								),
							),
						),
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'default_value'     => '',
						'placeholder'       => '',
						'prepend'           => '',
						'append'            => '',
						'maxlength'         => '',
					),
				),
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'page_type',
					'operator' => '==',
					'value'    => 'front_page',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => false,
		'description'           => '',
		'show_in_rest'          => 0,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5d6e2e973ceef',
		'title'                 => __( 'Blocks', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5d6e6525f88a5',
				'label'             => __( 'Content', 'lesverts' ),
				'name'              => 'content',
				'aria-label'        => '',
				'type'              => 'flexible_content',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'layouts'           => array(
					'layout_5d6e652ced572' => array(
						'key'        => 'layout_5d6e652ced572',
						'name'       => 'map',
						'label'      => __( 'Map', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5d6e6544f88a6',
								'label'             => '',
								'name'              => '',
								'aria-label'        => '',
								'type'              => 'clone',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'clone'             => array(
									0 => 'group_5d6e374bd3dcc',
								),
								'display'           => 'seamless',
								'layout'            => 'block',
								'prefix_label'      => 0,
								'prefix_name'       => 0,
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_5d6e83e0a5327' => array(
						'key'        => 'layout_5d6e83e0a5327',
						'name'       => 'code',
						'label'      => __( 'Code', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5d6e83efa5328',
								'label'             => '',
								'name'              => '',
								'aria-label'        => '',
								'type'              => 'clone',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'clone'             => array(
									0 => 'field_5c4f21cbd787e',
								),
								'display'           => 'seamless',
								'layout'            => 'block',
								'prefix_label'      => 0,
								'prefix_name'       => 0,
							),
						),
						'min'        => '',
						'max'        => '',
					),
				),
				'button_label'      => __( 'Add Block', 'lesverts' ),
				'min'               => 1,
				'max'               => '',
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'block',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => array(
			0  => 'permalink',
			1  => 'the_content',
			2  => 'excerpt',
			3  => 'discussion',
			4  => 'comments',
			5  => 'slug',
			6  => 'author',
			7  => 'format',
			8  => 'page_attributes',
			9  => 'featured_image',
			10 => 'categories',
			11 => 'tags',
			12 => 'send-trackbacks',
		),
		'active'                => true,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1567523863,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b6ac8b4171c0',
		'title'                 => __( 'Event Description', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5b6ac8da50013',
				'label'             => __( 'Image', 'lesverts' ),
				'name'              => 'image',
				'aria-label'        => '',
				'type'              => 'image',
				'instructions'      => __( 'Displayed in the teaser only.', 'lesverts' ),
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'return_format'     => 'array',
				'preview_size'      => 'thumbnail',
				'library'           => 'all',
				'min_width'         => '',
				'min_height'        => '',
				'min_size'          => '',
				'max_width'         => '',
				'max_height'        => '',
				'max_size'          => '',
				'mime_types'        => '',
				'translations'      => 'copy_once',
			),
			array(
				'key'               => 'field_62e238b3b87a7',
				'label'             => __( 'Content', 'lesverts' ),
				'name'              => 'event_content',
				'aria-label'        => '',
				'type'              => 'clone',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'clone'             => array(
					0 => 'group_5cee958e0d362',
				),
				'display'           => 'seamless',
				'layout'            => 'block',
				'prefix_label'      => 0,
				'prefix_name'       => 0,
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'tribe_events',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => array(
			0  => 'the_content',
			1  => 'excerpt',
			2  => 'discussion',
			3  => 'comments',
			4  => 'slug',
			5  => 'author',
			6  => 'format',
			7  => 'page_attributes',
			8  => 'featured_image',
			9  => 'categories',
			10 => 'send-trackbacks',
		),
		'active'                => true,
		'description'           => '',
		'show_in_rest'          => 0,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b646018d7f1a',
		'title'                 => __( 'Front page', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5b646034e8b37',
				'label'             => __( 'Main Campaign', 'lesverts' ),
				'name'              => 'campaign',
				'aria-label'        => '',
				'type'              => 'clone',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'clone'             => array(
					0 => 'group_5b645b16b8a08',
				),
				'display'           => 'group',
				'layout'            => 'block',
				'prefix_label'      => 0,
				'prefix_name'       => 1,
			),
			array(
				'key'               => 'field_5b684af179d22',
				'label'             => __( 'Content Blocks', 'lesverts' ),
				'name'              => 'content_blocks',
				'aria-label'        => '',
				'type'              => 'flexible_content',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'layouts'           => array(
					'5b684af8c6b51'        => array(
						'key'        => '5b684af8c6b51',
						'name'       => 'vote',
						'label'      => __( 'Vote Recommendations', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b684cf979d23',
								'label'             => __( 'Vote Recommendation', 'lesverts' ),
								'name'              => 'vote',
								'aria-label'        => '',
								'type'              => 'clone',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'clone'             => array(
									0 => 'group_5b6841bfef6ab',
								),
								'display'           => 'seamless',
								'layout'            => 'block',
								'prefix_label'      => 0,
								'prefix_name'       => 0,
							),
						),
						'min'        => '0',
						'max'        => '1',
					),
					'layout_5b68527d1477a' => array(
						'key'        => 'layout_5b68527d1477a',
						'name'       => 'single',
						'label'      => __( 'Single Post / Page Teaser', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b6852ec1477b',
								'label'             => __( 'Post / Page', 'lesverts' ),
								'name'              => 'single',
								'aria-label'        => '',
								'type'              => 'clone',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'clone'             => array(
									0 => 'group_5b6846d09aa72',
								),
								'display'           => 'seamless',
								'layout'            => 'block',
								'prefix_label'      => 0,
								'prefix_name'       => 0,
							),
						),
						'min'        => '0',
						'max'        => '4',
					),
					'layout_5b68533d5538a' => array(
						'key'        => 'layout_5b68533d5538a',
						'name'       => 'double',
						'label'      => __( 'Double Post / Page Teaser', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b68536b5538b',
								'label'             => __( 'Two Posts / Pages', 'lesverts' ),
								'name'              => 'double',
								'aria-label'        => '',
								'type'              => 'clone',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'clone'             => array(
									0 => 'group_5b684a4bdffb6',
								),
								'display'           => 'seamless',
								'layout'            => 'block',
								'prefix_label'      => 0,
								'prefix_name'       => 0,
							),
						),
						'min'        => '0',
						'max'        => '2',
					),
					'layout_5b6854e05538c' => array(
						'key'        => 'layout_5b6854e05538c',
						'name'       => 'media',
						'label'      => __( 'Latest Press Release', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b6854fa5538d',
								'label'             => __( 'Latest Press Release', 'lesverts' ),
								'name'              => 'media',
								'aria-label'        => '',
								'type'              => 'clone',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'clone'             => array(
									0 => 'group_5b68487f04a16',
								),
								'display'           => 'seamless',
								'layout'            => 'block',
								'prefix_label'      => 0,
								'prefix_name'       => 0,
							),
						),
						'min'        => '0',
						'max'        => '1',
					),
					'layout_5b6ae44c62e25' => array(
						'key'        => 'layout_5b6ae44c62e25',
						'name'       => 'events',
						'label'      => __( 'Events', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b6ae45562e26',
								'label'             => __( 'Events', 'lesverts' ),
								'name'              => 'events',
								'aria-label'        => '',
								'type'              => 'clone',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'clone'             => array(
									0 => 'group_5b6ae2eb01de7',
								),
								'display'           => 'seamless',
								'layout'            => 'block',
								'prefix_label'      => 0,
								'prefix_name'       => 0,
							),
						),
						'min'        => '0',
						'max'        => '1',
					),
					'layout_5b6b17209a92c' => array(
						'key'        => 'layout_5b6b17209a92c',
						'name'       => 'positions',
						'label'      => __( 'Positions', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b6b17259a92d',
								'label'             => __( 'Positions', 'lesverts' ),
								'name'              => 'positions',
								'aria-label'        => '',
								'type'              => 'clone',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'clone'             => array(
									0 => 'group_5b6b162bb52ed',
								),
								'display'           => 'seamless',
								'layout'            => 'block',
								'prefix_label'      => 0,
								'prefix_name'       => 0,
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_5b6c6a2d64762' => array(
						'key'        => 'layout_5b6c6a2d64762',
						'name'       => 'testimonials',
						'label'      => __( 'Testimonials', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b6c6a3764763',
								'label'             => __( 'Testimonials', 'lesverts' ),
								'name'              => 'testimonials',
								'aria-label'        => '',
								'type'              => 'clone',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'clone'             => array(
									0 => 'group_5b6c6072e2d3f',
								),
								'display'           => 'seamless',
								'layout'            => 'block',
								'prefix_label'      => 0,
								'prefix_name'       => 0,
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_5b6dad5b864d5' => array(
						'key'        => 'layout_5b6dad5b864d5',
						'name'       => 'engagement_map',
						'label'      => __( 'Map Block', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b6dad6b864d6',
								'label'             => __( 'Map Block', 'lesverts' ),
								'name'              => 'map_block',
								'aria-label'        => '',
								'type'              => 'clone',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'clone'             => array(
									0 => 'group_5b6da383ba62c',
								),
								'display'           => 'seamless',
								'layout'            => 'block',
								'prefix_label'      => 0,
								'prefix_name'       => 0,
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_5fdbdba216ca7' => array(
						'key'        => 'layout_5fdbdba216ca7',
						'name'       => 'block',
						'label'      => __( 'Block', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5fdbe573c8146',
								'label'             => '',
								'name'              => '',
								'aria-label'        => '',
								'type'              => 'clone',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'clone'             => array(
									0 => 'group_5fdbe45d039e4',
								),
								'display'           => 'seamless',
								'layout'            => 'block',
								'prefix_label'      => 0,
								'prefix_name'       => 0,
							),
						),
						'min'        => '',
						'max'        => '',
					),
				),
				'button_label'      => __( 'Add Content Block', 'lesverts' ),
				'min'               => '',
				'max'               => '',
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'page_type',
					'operator' => '==',
					'value'    => 'front_page',
				),
				array(
					'param'    => 'page_template',
					'operator' => '!=',
					'value'    => 'single_front.php',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'seamless',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => array(
			0  => 'the_content',
			1  => 'excerpt',
			2  => 'discussion',
			3  => 'comments',
			4  => 'slug',
			5  => 'author',
			6  => 'format',
			7  => 'page_attributes',
			8  => 'featured_image',
			9  => 'categories',
			10 => 'tags',
			11 => 'send-trackbacks',
		),
		'active'                => true,
		'description'           => '',
		'show_in_rest'          => false,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5fa5910a038f2',
		'title'                 => __( 'Page Template Front Page', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5fdbdf0cb82a3',
				'label'             => '',
				'name'              => '',
				'aria-label'        => '',
				'type'              => 'clone',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'clone'             => array(
					0 => 'group_5b646018d7f1a',
				),
				'display'           => 'seamless',
				'layout'            => 'block',
				'prefix_label'      => 0,
				'prefix_name'       => 0,
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'page_template',
					'operator' => '==',
					'value'    => 'single_front.php',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'seamless',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => array(
			0  => 'the_content',
			1  => 'excerpt',
			2  => 'discussion',
			3  => 'comments',
			4  => 'slug',
			5  => 'author',
			6  => 'format',
			7  => 'featured_image',
			8  => 'categories',
			9  => 'tags',
			10 => 'send-trackbacks',
		),
		'active'                => true,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1608245051,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b6c0884eb9b8',
		'title'                 => __( 'Person', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5bb4e39c23f7a',
				'label'             => __( 'Full Name', 'lesverts' ),
				'name'              => 'full_name',
				'aria-label'        => '',
				'type'              => 'text',
				'instructions'      => '',
				'required'          => 1,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'default_value'     => '',
				'placeholder'       => '',
				'prepend'           => '',
				'append'            => '',
				'maxlength'         => '',
			),
			array(
				'key'               => 'field_5b6c08b39417e',
				'label'             => __( 'Portrait Photo', 'lesverts' ),
				'name'              => 'image',
				'aria-label'        => '',
				'type'              => 'image',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'return_format'     => 'array',
				'preview_size'      => 'thumbnail',
				'library'           => 'all',
				'min_width'         => 720,
				'min_height'        => 720,
				'min_size'          => '',
				'max_width'         => '',
				'max_height'        => '',
				'max_size'          => '',
				'mime_types'        => '',
			),
			array(
				'key'               => 'field_5b6c09509417f',
				'label'             => __( 'Contact', 'lesverts' ),
				'name'              => 'contact',
				'aria-label'        => '',
				'type'              => 'flexible_content',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'layouts'           => array(
					'5b6c09700cad3'        => array(
						'key'        => '5b6c09700cad3',
						'name'       => 'email',
						'label'      => __( 'Email', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b6c097794180',
								'label'             => __( 'Email', 'lesverts' ),
								'name'              => 'email',
								'aria-label'        => '',
								'type'              => 'text',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'placeholder'       => '',
								'prepend'           => '',
								'append'            => '',
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_5b6c09a694181' => array(
						'key'        => 'layout_5b6c09a694181',
						'name'       => 'phone',
						'label'      => __( 'Phone', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b6c09b294182',
								'label'             => __( 'Phone', 'lesverts' ),
								'name'              => 'phone',
								'aria-label'        => '',
								'type'              => 'text',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'placeholder'       => '',
								'prepend'           => '',
								'append'            => '',
								'maxlength'         => '',
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_5b6c09ca94183' => array(
						'key'        => 'layout_5b6c09ca94183',
						'name'       => 'website',
						'label'      => __( 'Website', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b6c09d394184',
								'label'             => __( 'Website', 'lesverts' ),
								'name'              => 'website',
								'aria-label'        => '',
								'type'              => 'url',
								'instructions'      => '',
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'placeholder'       => '',
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_5b6c0a1e94186' => array(
						'key'        => 'layout_5b6c0a1e94186',
						'name'       => 'twitter',
						'label'      => __( 'Twitter', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b6c0a2494187',
								'label'             => __( 'Twitter Name', 'lesverts' ),
								'name'              => 'twitter',
								'aria-label'        => '',
								'type'              => 'text',
								'instructions'      => __( 'Use @name', 'lesverts' ),
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'placeholder'       => '',
								'prepend'           => '',
								'append'            => '',
								'maxlength'         => '',
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_5b6c0a5b94188' => array(
						'key'        => 'layout_5b6c0a5b94188',
						'name'       => 'facebook',
						'label'      => __( 'Facebook', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b6c0a6294189',
								'label'             => __( 'Facebook', 'lesverts' ),
								'name'              => 'facebook',
								'aria-label'        => '',
								'type'              => 'url',
								'instructions'      => __( 'Use the URL to the profile or page', 'lesverts' ),
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'placeholder'       => '',
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_bluesky_editor' => array(
						'key'        => 'layout_bluesky_editor',
						'name'       => 'bluesky',
						'label'      => __( 'Bluesky', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_bluesky_editor',
								'label'             => __( 'Bluesky Name', 'lesverts' ),
								'name'              => 'bluesky',
								'aria-label'        => '',
								'type'              => 'text',
								'instructions'      => __( 'Use @name', 'lesverts' ),
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'placeholder'       => '',
								'prepend'           => '',
								'append'            => '',
								'maxlength'         => '',
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_tiktok_editor' => array(
						'key'        => 'layout_tiktok_editor',
						'name'       => 'tiktok',
						'label'      => __( 'Tiktok', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_tiktok_editor',
								'label'             => __( 'Tiktok Name', 'lesverts' ),
								'name'              => 'tiktok',
								'aria-label'        => '',
								'type'              => 'text',
								'instructions'      => __( 'Use @name', 'lesverts' ),
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'placeholder'       => '',
								'prepend'           => '',
								'append'            => '',
								'maxlength'         => '',
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_5b6c0a909418a' => array(
						'key'        => 'layout_5b6c0a909418a',
						'name'       => 'instagram',
						'label'      => __( 'Instagram', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_5b6c0ad89418b',
								'label'             => __( 'Instagram Name', 'lesverts' ),
								'name'              => 'insta',
								'aria-label'        => '',
								'type'              => 'text',
								'instructions'      => __( 'Use @name', 'lesverts' ),
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'placeholder'       => '',
								'prepend'           => '',
								'append'            => '',
								'maxlength'         => '',
							),
						),
						'min'        => '',
						'max'        => '1',
					),
					'layout_threads_editor' => array(
						'key'        => 'layout_threads_editor',
						'name'       => 'threads',
						'label'      => __( 'Threads', 'lesverts' ),
						'display'    => 'block',
						'sub_fields' => array(
							array(
								'key'               => 'field_threads_editor',
								'label'             => __( 'Threads Name', 'lesverts' ),
								'name'              => 'threads',
								'aria-label'        => '',
								'type'              => 'text',
								'instructions'      => __( 'Use @name', 'lesverts' ),
								'required'          => 0,
								'conditional_logic' => 0,
								'wrapper'           => array(
									'width' => '',
									'class' => '',
									'id'    => '',
								),
								'default_value'     => '',
								'placeholder'       => '',
								'prepend'           => '',
								'append'            => '',
								'maxlength'         => '',
							),
						),
						'min'        => '',
						'max'        => '1',
					),
				),
				'button_label'      => __( 'Add Contact', 'lesverts' ),
				'min'               => '',
				'max'               => '',
			),
			array(
				'key'               => 'field_5bbb2b59ceb1f',
				'label'             => __( 'Testimonials', 'lesverts' ),
				'name'              => 'testimonials',
				'aria-label'        => '',
				'type'              => 'repeater',
				'instructions'      => __( 'Displayed by the testimonials block (≠ quote block).', 'lesverts' ),
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'collapsed'         => 'field_5bbb2a71ceb1d',
				'min'               => 0,
				'max'               => 0,
				'layout'            => 'block',
				'button_label'      => __( 'Add Testimonial', 'lesverts' ),
				'sub_fields'        => array(
					array(
						'key'               => 'field_5bb4e16f39fc2',
						'label'             => __( 'Quote', 'lesverts' ),
						'name'              => 'quote',
						'aria-label'        => '',
						'type'              => 'textarea',
						'instructions'      => __( 'Max 280 characters', 'lesverts' ),
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'default_value'     => '',
						'placeholder'       => '',
						'maxlength'         => 280,
						'rows'              => 3,
						'new_lines'         => '',
						'translations'      => 'ignore',
						'parent_repeater'   => 'field_5bbb2b59ceb1f',
					),
					array(
						'key'               => 'field_5bb4e14d39fc1',
						'label'             => __( 'Role', 'lesverts' ),
						'name'              => 'role',
						'aria-label'        => '',
						'type'              => 'text',
						'instructions'      => __( 'Optional. Example: President Greens Fribourg, Business owner', 'lesverts' ),
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'default_value'     => '',
						'placeholder'       => '',
						'prepend'           => '',
						'append'            => '',
						'maxlength'         => '',
						'parent_repeater'   => 'field_5bbb2b59ceb1f',
					),
					array(
						'key'               => 'field_5bbb2a71ceb1d',
						'label'             => __( 'Testimonial Category', 'lesverts' ),
						'name'              => 'taxonomy',
						'aria-label'        => '',
						'type'              => 'taxonomy',
						'instructions'      => __( 'To present testimonials you will have to select the testimonials by category, so <strong>assign the same category to all testimonials you want to display in the same collection</strong>.', 'lesverts' ),
						'required'          => 1,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'taxonomy'          => 'testimonials',
						'field_type'        => 'multi_select',
						'allow_null'        => 0,
						'add_term'          => 1,
						'save_terms'        => 0,
						'load_terms'        => 0,
						'return_format'     => 'id',
						'multiple'          => 0,
						'parent_repeater'   => 'field_5bbb2b59ceb1f',
					),
				),
				'rows_per_page'     => 20,
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'people',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => array(
			0  => 'the_content',
			1  => 'excerpt',
			2  => 'discussion',
			3  => 'comments',
			4  => 'revisions',
			5  => 'slug',
			6  => 'author',
			7  => 'format',
			8  => 'page_attributes',
			9  => 'featured_image',
			10 => 'categories',
			11 => 'tags',
			12 => 'send-trackbacks',
		),
		'active'                => true,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1614790852,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b6c01111578c',
		'title'                 => __( 'Press Release', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5b9b7511d846f',
				'label'             => __( 'Lead', 'lesverts' ),
				'name'              => '',
				'aria-label'        => '',
				'type'              => 'clone',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'clone'             => array(
					0 => 'field_5bcee7167c5c5',
				),
				'display'           => 'group',
				'layout'            => 'block',
				'prefix_label'      => 0,
				'prefix_name'       => 0,
			),
			array(
				'key'               => 'field_5b9b73df97fd2',
				'label'             => __( 'Main Content', 'lesverts' ),
				'name'              => 'main_content',
				'aria-label'        => '',
				'type'              => 'group',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'layout'            => 'block',
				'sub_fields'        => array(
					array(
						'key'               => 'field_5b6c055d04c40',
						'label'             => __( 'Content', 'lesverts' ),
						'name'              => 'content',
						'aria-label'        => '',
						'type'              => 'clone',
						'instructions'      => '',
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'clone'             => array(
							0 => 'group_5cee958e0d362',
						),
						'display'           => 'seamless',
						'layout'            => 'block',
						'prefix_label'      => 0,
						'prefix_name'       => 0,
					),
				),
			),
			array(
				'key'               => 'field_5b6c4c5cb2fea',
				'label'             => __( 'Link Lists', 'lesverts' ),
				'name'              => 'link_lists',
				'aria-label'        => '',
				'type'              => 'group',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'layout'            => 'block',
				'sub_fields'        => array(
					array(
						'key'               => 'field_5b9b7369e4072',
						'label'             => __( 'Link List', 'lesverts' ),
						'name'              => 'linked_content',
						'aria-label'        => '',
						'type'              => 'clone',
						'instructions'      => '',
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'clone'             => array(
							0 => 'group_5b6c4bfaa8a26',
						),
						'display'           => 'seamless',
						'layout'            => 'block',
						'prefix_label'      => 0,
						'prefix_name'       => 0,
					),
				),
			),
			array(
				'key'               => 'field_5b9b758b2e6b6',
				'label'             => __( 'Settings', 'lesverts' ),
				'name'              => 'settings',
				'aria-label'        => '',
				'type'              => 'group',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'layout'            => 'block',
				'sub_fields'        => array(
					array(
						'key'               => 'field_5b6c01dc3a2b9',
						'label'             => __( 'Show on front page', 'lesverts' ),
						'name'              => 'show_on_front_page',
						'aria-label'        => '',
						'type'              => 'true_false',
						'instructions'      => __( 'Set this to false, to hide it from the \'Latest Press Release\' block on the front page.', 'lesverts' ),
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'message'           => '',
						'default_value'     => 1,
						'ui'                => 1,
						'ui_on_text'        => '',
						'ui_off_text'       => '',
					),
				),
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'post_template',
					'operator' => '==',
					'value'    => 'single_press_release.php',
				),
			),
		),
		'menu_order'            => 0,
		'position'              => 'normal',
		'style'                 => 'seamless',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => array(
			0 => 'the_content',
			1 => 'excerpt',
			2 => 'discussion',
			3 => 'comments',
			4 => 'slug',
			5 => 'author',
			6 => 'format',
			7 => 'featured_image',
			8 => 'send-trackbacks',
		),
		'active'                => true,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1567419960,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b6c3c5890c40',
		'title'                 => __( 'Default', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5b7c3141824bf',
				'label'             => __( 'Teaser', 'lesverts' ),
				'name'              => '',
				'aria-label'        => '',
				'type'              => 'clone',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'clone'             => array(
					0 => 'group_5bcee6dd2cfd7',
				),
				'display'           => 'group',
				'layout'            => 'block',
				'prefix_label'      => 0,
				'prefix_name'       => 0,
			),
			array(
				'key'               => 'field_5b7c2cb8fb969',
				'label'             => __( 'Main Content', 'lesverts' ),
				'name'              => 'main_content',
				'aria-label'        => '',
				'type'              => 'group',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'layout'            => 'block',
				'sub_fields'        => array(
					array(
						'key'               => 'field_5b6c3dd87bf2a',
						'label'             => __( 'Header Image', 'lesverts' ),
						'name'              => 'header_image',
						'aria-label'        => '',
						'type'              => 'image',
						'instructions'      => '',
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'return_format'     => 'array',
						'preview_size'      => 'regular',
						'library'           => 'all',
						'min_width'         => '',
						'min_height'        => '',
						'min_size'          => '',
						'max_width'         => '',
						'max_height'        => '',
						'max_size'          => '',
						'mime_types'        => '',
					),
					array(
						'key'               => 'field_5cee97780b1eb',
						'label'             => __( 'Content', 'lesverts' ),
						'name'              => 'content',
						'aria-label'        => '',
						'type'              => 'clone',
						'instructions'      => '',
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'clone'             => array(
							0 => 'group_5cee958e0d362',
						),
						'display'           => 'seamless',
						'layout'            => 'block',
						'prefix_label'      => 0,
						'prefix_name'       => 0,
					),
				),
			),
			array(
				'key'               => 'field_5b7c2ce4fb96a',
				'label'             => __( 'Link Lists', 'lesverts' ),
				'name'              => 'link_lists',
				'aria-label'        => '',
				'type'              => 'group',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'layout'            => 'block',
				'sub_fields'        => array(
					array(
						'key'               => 'field_5b6c4ce9059a7',
						'label'             => __( 'Link List', 'lesverts' ),
						'name'              => 'linked_content',
						'aria-label'        => '',
						'type'              => 'clone',
						'instructions'      => '',
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'clone'             => array(
							0 => 'group_5b6c4bfaa8a26',
						),
						'display'           => 'seamless',
						'layout'            => 'block',
						'prefix_label'      => 0,
						'prefix_name'       => 0,
					),
				),
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'post',
				),
				array(
					'param'    => 'post_template',
					'operator' => '==',
					'value'    => 'default',
				),
			),
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'page',
				),
				array(
					'param'    => 'post_template',
					'operator' => '==',
					'value'    => 'default',
				),
				array(
					'param'    => 'page_type',
					'operator' => '!=',
					'value'    => 'front_page',
				),
				array(
					'param'    => 'page_type',
					'operator' => '!=',
					'value'    => 'posts_page',
				),
			),
		),
		'menu_order'            => 10,
		'position'              => 'normal',
		'style'                 => 'seamless',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => array(
			0 => 'the_content',
			1 => 'excerpt',
			2 => 'discussion',
			3 => 'comments',
			4 => 'revisions',
			5 => 'slug',
			6 => 'author',
			7 => 'format',
			8 => 'featured_image',
			9 => 'send-trackbacks',
		),
		'active'                => true,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1625750667,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_60e6fcf5eff75',
		'title'                 => __( 'Landing Page', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_60e6fcf637d55',
				'label'             => __( 'Teaser', 'lesverts' ),
				'name'              => '',
				'aria-label'        => '',
				'type'              => 'clone',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'clone'             => array(
					0 => 'group_5bcee6dd2cfd7',
				),
				'display'           => 'group',
				'layout'            => 'block',
				'prefix_label'      => 0,
				'prefix_name'       => 0,
			),
			array(
				'key'               => 'field_60e6fcf637d90',
				'label'             => __( 'Main Content', 'lesverts' ),
				'name'              => 'main_content',
				'aria-label'        => '',
				'type'              => 'group',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'layout'            => 'block',
				'sub_fields'        => array(
					array(
						'key'               => 'field_60e6fcf654e7f',
						'label'             => __( 'Header Image', 'lesverts' ),
						'name'              => 'header_image',
						'aria-label'        => '',
						'type'              => 'image',
						'instructions'      => '',
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'return_format'     => 'array',
						'preview_size'      => 'regular',
						'library'           => 'all',
						'min_width'         => '',
						'min_height'        => '',
						'min_size'          => '',
						'max_width'         => '',
						'max_height'        => '',
						'max_size'          => '',
						'mime_types'        => '',
					),
					array(
						'key'               => 'field_60e6fcf654eb9',
						'label'             => __( 'Content', 'lesverts' ),
						'name'              => 'content',
						'aria-label'        => '',
						'type'              => 'clone',
						'instructions'      => '',
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'clone'             => array(
							0 => 'group_5cee958e0d362',
						),
						'display'           => 'seamless',
						'layout'            => 'block',
						'prefix_label'      => 0,
						'prefix_name'       => 0,
					),
				),
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'post',
				),
				array(
					'param'    => 'post_template',
					'operator' => '==',
					'value'    => 'single_landingpage.php',
				),
			),
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'page',
				),
				array(
					'param'    => 'post_template',
					'operator' => '==',
					'value'    => 'single_landingpage.php',
				),
			),
		),
		'menu_order'            => 10,
		'position'              => 'normal',
		'style'                 => 'seamless',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => array(
			0 => 'the_content',
			1 => 'excerpt',
			2 => 'discussion',
			3 => 'comments',
			4 => 'revisions',
			5 => 'slug',
			6 => 'format',
			7 => 'featured_image',
			8 => 'send-trackbacks',
		),
		'active'                => true,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1626181174,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b9a8a0bca273',
		'title'                 => __( 'Overview', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5b9a8a0be9449',
				'label'             => __( 'Lead', 'lesverts' ),
				'name'              => '',
				'aria-label'        => '',
				'type'              => 'clone',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'clone'             => array(
					0 => 'field_5bcee7167c5c5',
				),
				'display'           => 'group',
				'layout'            => 'block',
				'prefix_label'      => 0,
				'prefix_name'       => 0,
			),
			array(
				'key'               => 'field_5b9a8a0be959d',
				'label'             => __( 'Main Content', 'lesverts' ),
				'name'              => 'posts',
				'aria-label'        => '',
				'type'              => 'repeater',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'collapsed'         => 'field_5b9a8b184a95e',
				'min'               => 1,
				'max'               => 0,
				'layout'            => 'table',
				'button_label'      => __( 'Add Post / Page', 'lesverts' ),
				'sub_fields'        => array(
					array(
						'key'               => 'field_5b9a8b184a95e',
						'label'             => __( 'Post / Page', 'lesverts' ),
						'name'              => 'post',
						'aria-label'        => '',
						'type'              => 'post_object',
						'instructions'      => __( 'Select the post or page, that should be displayed.', 'lesverts' ),
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'post_type'         => array(
							0 => 'post',
							1 => 'page',
						),
						'taxonomy'          => array(),
						'allow_null'        => 0,
						'multiple'          => 0,
						'return_format'     => 'id',
						'ui'                => 1,
						'parent_repeater'   => 'field_5b9a8a0be959d',
					),
				),
				'rows_per_page'     => 20,
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'post',
				),
				array(
					'param'    => 'post_template',
					'operator' => '==',
					'value'    => 'single_overview.php',
				),
			),
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'page',
				),
				array(
					'param'    => 'page_template',
					'operator' => '==',
					'value'    => 'single_overview.php',
				),
			),
		),
		'menu_order'            => 10,
		'position'              => 'normal',
		'style'                 => 'seamless',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => array(
			0 => 'the_content',
			1 => 'excerpt',
			2 => 'discussion',
			3 => 'comments',
			4 => 'revisions',
			5 => 'slug',
			6 => 'author',
			7 => 'format',
			8 => 'featured_image',
			9 => 'send-trackbacks',
		),
		'active'                => true,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1569833931,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5f2846b8b4078',
		'title'                 => __( 'Posts Page', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5f2846b91a433',
				'label'             => __( 'Teaser', 'lesverts' ),
				'name'              => '',
				'aria-label'        => '',
				'type'              => 'clone',
				'instructions'      => '',
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'clone'             => array(
					0 => 'field_5bcee7167c5c5',
				),
				'display'           => 'group',
				'layout'            => 'block',
				'prefix_label'      => 0,
				'prefix_name'       => 0,
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'page_type',
					'operator' => '==',
					'value'    => 'posts_page',
				),
			),
		),
		'menu_order'            => 10,
		'position'              => 'normal',
		'style'                 => 'seamless',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => array(
			0 => 'the_content',
			1 => 'excerpt',
			2 => 'discussion',
			3 => 'comments',
			4 => 'revisions',
			5 => 'slug',
			6 => 'author',
			7 => 'format',
			8 => 'featured_image',
			9 => 'send-trackbacks',
		),
		'active'                => true,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1596475240,
	) );

	acf_add_local_field_group( array(
		'key'                   => 'group_5b76febf369b4',
		'title'                 => __( 'Social Media', 'lesverts' ),
		'fields'                => array(
			array(
				'key'               => 'field_5b76fef28095e',
				'label'             => __( 'Display Share Buttons', 'lesverts' ),
				'name'              => 'share_buttons',
				'aria-label'        => '',
				'type'              => 'true_false',
				'instructions'      => __( 'You can hide the share buttons of this page, by disabling this option.', 'lesverts' ),
				'required'          => 0,
				'conditional_logic' => 0,
				'wrapper'           => array(
					'width' => '',
					'class' => '',
					'id'    => '',
				),
				'message'           => '',
				'default_value'     => 1,
				'ui'                => 1,
				'ui_on_text'        => '',
				'ui_off_text'       => '',
			),
		),
		'location'              => array(
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'post',
				),
			),
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'page',
				),
				array(
					'param'    => 'page_type',
					'operator' => '!=',
					'value'    => 'front_page',
				),
				array(
					'param'    => 'page_type',
					'operator' => '!=',
					'value'    => 'posts_page',
				),
				array(
					'param'    => 'page_template',
					'operator' => '==',
					'value'    => 'default',
				),
			),
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'tribe_events',
				),
			),
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'post',
				),
				array(
					'param'    => 'post_template',
					'operator' => '==',
					'value'    => 'single_landingpage.php',
				),
			),
			array(
				array(
					'param'    => 'post_type',
					'operator' => '==',
					'value'    => 'page',
				),
				array(
					'param'    => 'page_template',
					'operator' => '==',
					'value'    => 'single_landingpage.php',
				),
			),
		),
		'menu_order'            => 20,
		'position'              => 'normal',
		'style'                 => 'default',
		'label_placement'       => 'top',
		'instruction_placement' => 'label',
		'hide_on_screen'        => '',
		'active'                => true,
		'description'           => '',
		'show_in_rest'          => false,
		'modified'              => 1625750433,
	) );

	add_action( 'acf/include_fields', function() {
	if ( ! function_exists( 'acf_add_local_field_group' ) ) {
		return;
	}

	acf_add_local_field_group( array(
	'key' => 'group_5fa587aac61ec',
	'title' => '(Category) Author',
	'fields' => array(
		array(
			'key' => 'field_5fa587ab352a8',
			'label' => __( 'Show author', 'lesverts' ),
			'name' => 'has_author',
			'aria-label' => '',
			'type' => 'true_false',
			'instructions' => __( 'Show the author for all posts of this category?', 'lesverts' ),
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'message' => '',
			'default_value' => 0,
			'ui' => 1,
			'ui_on_text' => '',
			'ui_off_text' => '',
		),
	),
	'location' => array(
		array(
			array(
				'param' => 'taxonomy',
				'operator' => '==',
				'value' => 'category',
			),
		),
	),
	'menu_order' => 0,
	'position' => 'normal',
	'style' => 'default',
	'label_placement' => 'top',
	'instruction_placement' => 'label',
	'hide_on_screen' => '',
	'active' => true,
	'description' => '',
	'show_in_rest' => false,
	'display_title' => '',
	'modified' => 1604683888,
) );

	acf_add_local_field_group( array(
	'key' => 'group_5c5ab079896c0',
	'title' => '(Category) Date',
	'fields' => array(
		array(
			'key' => 'field_5c5ab09d6c3f1',
			'label' => __( 'Has date', 'lesverts' ),
			'name' => 'has_date',
			'aria-label' => '',
			'type' => 'true_false',
			'instructions' => __( 'Should a date be displayed with all posts categorized in this category?', 'lesverts' ),
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'message' => '',
			'default_value' => 0,
			'ui' => 1,
			'ui_on_text' => '',
			'ui_off_text' => '',
		),
	),
	'location' => array(
		array(
			array(
				'param' => 'taxonomy',
				'operator' => '==',
				'value' => 'category',
			),
		),
	),
	'menu_order' => 0,
	'position' => 'normal',
	'style' => 'default',
	'label_placement' => 'top',
	'instruction_placement' => 'label',
	'hide_on_screen' => '',
	'active' => true,
	'description' => '',
	'show_in_rest' => false,
	'display_title' => '',
	'modified' => 1549460588,
) );

	acf_add_local_field_group( array(
	'key' => 'group_tracking_scripts_options',
	'title' => 'Tracking Scripts (Global)',
	'fields' => array(
		array(
			'key' => 'field_tracking_facebook_pixel_id',
			'label' => __( 'Facebook Pixel ID', 'lesverts' ),
			'name' => 'tracking_facebook_pixel_id',
			'aria-label' => '',
			'type' => 'text',
			'instructions' => __( 'Enter your Facebook Pixel ID (e.g., 1234567890)', 'lesverts' ),
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'default_value' => '',
			'placeholder' => 1234567890,
			'prepend' => '',
			'append' => '',
			'maxlength' => '',
		),
		array(
			'key' => 'field_tracking_custom_scripts',
			'label' => __( 'Custom Scripts', 'lesverts' ),
			'name' => 'tracking_custom_scripts',
			'aria-label' => '',
			'type' => 'repeater',
			'instructions' => __( 'Add custom tracking scripts that will be loaded via external URL', 'lesverts' ),
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'collapsed' => 'field_tracking_script_name',
			'min' => 0,
			'max' => 0,
			'layout' => 'table',
			'button_label' => __( 'Add Script', 'lesverts' ),
			'rows_per_page' => 20,
			'sub_fields' => array(
				array(
					'key' => 'field_tracking_script_name',
					'label' => __( 'Script Name', 'lesverts' ),
					'name' => 'script_name',
					'aria-label' => '',
					'type' => 'text',
					'instructions' => __( 'Name for admin reference (e.g., "Google Analytics")', 'lesverts' ),
					'required' => 1,
					'conditional_logic' => 0,
					'wrapper' => array(
						'width' => '',
						'class' => '',
						'id' => '',
					),
					'default_value' => '',
					'placeholder' => 'Google Analytics',
					'prepend' => '',
					'append' => '',
					'maxlength' => '',
					'parent_repeater' => 'field_tracking_custom_scripts',
				),
				array(
					'key' => 'field_tracking_script_url',
					'label' => __( 'Script URL', 'lesverts' ),
					'name' => 'script_url',
					'aria-label' => '',
					'type' => 'url',
					'instructions' => __( 'Full URL to the external script', 'lesverts' ),
					'required' => 1,
					'conditional_logic' => 0,
					'wrapper' => array(
						'width' => '',
						'class' => '',
						'id' => '',
					),
					'default_value' => '',
					'placeholder' => 'https://example.com/script.js',
					'parent_repeater' => 'field_tracking_custom_scripts',
				),
			),
		),
	),
	'location' => array(
		array(
			array(
				'param' => 'options_page',
				'operator' => '==',
				'value' => 'tracking-scripts-settings',
			),
		),
	),
	'menu_order' => 0,
	'position' => 'normal',
	'style' => 'default',
	'label_placement' => 'top',
	'instruction_placement' => 'label',
	'hide_on_screen' => '',
	'active' => true,
	'description' => 'Global tracking scripts configuration (same for all languages)',
	'show_in_rest' => 0,
	'display_title' => '',
) );

	acf_add_local_field_group( array(
	'key' => 'group_tracking_scripts_translatable',
	'title' => 'Tracking Scripts (Translatable)',
	'fields' => array(
		array(
			'key' => 'field_tracking_banner_text_translatable',
			'label' => __( 'Banner Text', 'lesverts' ),
			'name' => 'tracking_banner_text',
			'aria-label' => '',
			'type' => 'textarea',
			'instructions' => __( 'Text displayed in the cookie banner (language-specific)', 'lesverts' ),
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'default_value' => '',
			'placeholder' => 'Wir verwenden Tracking-Skripte...',
			'maxlength' => '',
			'rows' => 3,
			'new_lines' => '',
		),
		array(
			'key' => 'field_tracking_privacy_link_translatable',
			'label' => __( 'Privacy Policy Link', 'lesverts' ),
			'name' => 'tracking_privacy_link',
			'aria-label' => '',
			'type' => 'url',
			'instructions' => __( 'Link to your privacy policy page (language-specific)', 'lesverts' ),
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'default_value' => '',
			'placeholder' => 'https://example.com/privacy',
		),
	),
	'location' => array(
		array(
			array(
				'param' => 'options_page',
				'operator' => '==',
				'value' => 'tracking-scripts-de',
			),
		),
		array(
			array(
				'param' => 'options_page',
				'operator' => '==',
				'value' => 'tracking-scripts-fr',
			),
		),
	),
	'menu_order' => 0,
	'position' => 'normal',
	'style' => 'default',
	'label_placement' => 'top',
	'instruction_placement' => 'label',
	'hide_on_screen' => '',
	'active' => true,
	'description' => 'Language-specific cookie banner text and privacy policy link',
	'show_in_rest' => 0,
	'display_title' => '',
) );

	acf_add_local_field_group( array(
	'key' => 'group_tracking_scripts_page',
	'title' => 'Enable Tracking Scripts',
	'fields' => array(
		array(
			'key' => 'field_enable_facebook_pixel',
			'label' => __( 'Enable Facebook Pixel', 'lesverts' ),
			'name' => 'enable_facebook_pixel',
			'aria-label' => '',
			'type' => 'true_false',
			'instructions' => __( 'Load Facebook Pixel on this page (requires Facebook Pixel ID to be configured in Tracking Scripts settings)', 'lesverts' ),
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'message' => __( 'Load Facebook Pixel on this page', 'lesverts' ),
			'default_value' => 0,
			'ui' => 1,
			'ui_on_text' => '',
			'ui_off_text' => '',
		),
		array(
			'key' => 'field_enable_custom_script_0',
			'label' => __( 'Enable Custom Script 1', 'lesverts' ),
			'name' => 'enable_custom_script_0',
			'aria-label' => '',
			'type' => 'true_false',
			'instructions' => __( 'Load the first custom script configured in Tracking Scripts settings', 'lesverts' ),
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'message' => __( 'Load Custom Script 1 on this page', 'lesverts' ),
			'default_value' => 0,
			'ui' => 1,
			'ui_on_text' => '',
			'ui_off_text' => '',
		),
		array(
			'key' => 'field_enable_custom_script_1',
			'label' => __( 'Enable Custom Script 2', 'lesverts' ),
			'name' => 'enable_custom_script_1',
			'aria-label' => '',
			'type' => 'true_false',
			'instructions' => __( 'Load the second custom script configured in Tracking Scripts settings', 'lesverts' ),
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'message' => __( 'Load Custom Script 2 on this page', 'lesverts' ),
			'default_value' => 0,
			'ui' => 1,
			'ui_on_text' => '',
			'ui_off_text' => '',
		),
		array(
			'key' => 'field_enable_custom_script_2',
			'label' => __( 'Enable Custom Script 3', 'lesverts' ),
			'name' => 'enable_custom_script_2',
			'aria-label' => '',
			'type' => 'true_false',
			'instructions' => __( 'Load the third custom script configured in Tracking Scripts settings', 'lesverts' ),
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'message' => __( 'Load Custom Script 3 on this page', 'lesverts' ),
			'default_value' => 0,
			'ui' => 1,
			'ui_on_text' => '',
			'ui_off_text' => '',
		),
		array(
			'key' => 'field_enable_custom_script_3',
			'label' => __( 'Enable Custom Script 4', 'lesverts' ),
			'name' => 'enable_custom_script_3',
			'aria-label' => '',
			'type' => 'true_false',
			'instructions' => __( 'Load the fourth custom script configured in Tracking Scripts settings', 'lesverts' ),
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'message' => __( 'Load Custom Script 4 on this page', 'lesverts' ),
			'default_value' => 0,
			'ui' => 1,
			'ui_on_text' => '',
			'ui_off_text' => '',
		),
		array(
			'key' => 'field_enable_custom_script_4',
			'label' => __( 'Enable Custom Script 5', 'lesverts' ),
			'name' => 'enable_custom_script_4',
			'aria-label' => '',
			'type' => 'true_false',
			'instructions' => __( 'Load the fifth custom script configured in Tracking Scripts settings', 'lesverts' ),
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'message' => __( 'Load Custom Script 5 on this page', 'lesverts' ),
			'default_value' => 0,
			'ui' => 1,
			'ui_on_text' => '',
			'ui_off_text' => '',
		),
	),
	'location' => array(
		array(
			array(
				'param' => 'post_type',
				'operator' => '==',
				'value' => 'post',
			),
		),
		array(
			array(
				'param' => 'post_type',
				'operator' => '==',
				'value' => 'page',
			),
		),
	),
	'menu_order' => 100,
	'position' => 'side',
	'style' => 'default',
	'label_placement' => 'top',
	'instruction_placement' => 'label',
	'hide_on_screen' => '',
	'active' => true,
	'description' => 'Enable tracking scripts for this page. The cookie banner will only appear if at least one script is enabled.',
	'show_in_rest' => 0,
	'display_title' => '',
) );
} );

add_action( 'acf/include_fields', function() {
	if ( ! function_exists( 'acf_add_local_field_group' ) ) {
		return;
	}

	acf_add_local_field_group( array(
	'key' => 'group_65e2a1b3c4d5e',
	'title' => '[Template]Initiatives Block',
	'fields' => array(
		array(
			'key' => 'field_65e2a1b3c4d5f',
			'label' => __( 'About this Block', 'lesverts' ),
			'name' => '',
			'aria-label' => '',
			'type' => 'message',
			'instructions' => '',
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'message' => __( 'Displays up to 4 images promoting your current initiatives. Images are displayed in a 4:1 ratio; recommended minimum size: 1000 x 250 px. The logo and title of the initiative should be part of the uploaded image. Please avoid images with a transparent background. The whole image is clickable and leads to the configured link.', 'lesverts' ),
			'new_lines' => 'wpautop',
			'esc_html' => 0,
		),
		array(
			'key' => 'field_65e2a1b3c4d60',
			'label' => __( 'Block Title', 'lesverts' ),
			'name' => 'block_title',
			'aria-label' => '',
			'type' => 'clone',
			'instructions' => '',
			'required' => 1,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'clone' => array(
				0 => 'group_5b684e748247e',
			),
			'display' => 'seamless',
			'layout' => 'block',
			'prefix_label' => 0,
			'prefix_name' => 0,
		),
		array(
			'key' => 'field_65e2a1b3c4d61',
			'label' => __( 'Initiatives', 'lesverts' ),
			'name' => 'rows',
			'aria-label' => '',
			'type' => 'repeater',
			'instructions' => __( 'Add up to 4 initiatives.', 'lesverts' ),
			'required' => 1,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'collapsed' => 'field_65e2a1b3c4d62',
			'min' => 1,
			'max' => 4,
			'layout' => 'block',
			'button_label' => __( 'Add Initiative', 'lesverts' ),
			'rows_per_page' => 20,
			'sub_fields' => array(
				array(
					'key' => 'field_65e2a1b3c4d62',
					'label' => __( 'Image', 'lesverts' ),
					'name' => 'image',
					'aria-label' => '',
					'type' => 'image',
					'instructions' => __( 'The logo and title of the initiative should be part of the uploaded image.', 'lesverts' ),
					'required' => 1,
					'conditional_logic' => 0,
					'wrapper' => array(
						'width' => '',
						'class' => '',
						'id' => '',
					),
					'return_format' => 'array',
					'preview_size' => 'medium',
					'library' => 'all',
					'min_width' => '',
					'min_height' => '',
					'min_size' => '',
					'max_width' => '',
					'max_height' => '',
					'max_size' => '',
					'mime_types' => '',
					'parent_repeater' => 'field_65e2a1b3c4d61',
				),
				array(
					'key' => 'field_65e2a1b3c4d65',
					'label' => __( 'Link', 'lesverts' ),
					'name' => 'link',
					'aria-label' => '',
					'type' => 'url',
					'instructions' => __( 'The whole image is clickable and leads to this link.', 'lesverts' ),
					'required' => 1,
					'conditional_logic' => 0,
					'wrapper' => array(
						'width' => '',
						'class' => '',
						'id' => '',
					),
					'default_value' => '',
					'placeholder' => '',
					'parent_repeater' => 'field_65e2a1b3c4d61',
				),
			),
		),
		array(
			'key' => 'field_65e2a1b3c4d70',
			'label' => __( 'Link to all Initiatives', 'lesverts' ),
			'name' => 'more_link',
			'aria-label' => '',
			'type' => 'group',
			'instructions' => __( 'Optional link displayed below the initiative images.', 'lesverts' ),
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'layout' => 'block',
			'sub_fields' => array(
				array(
					'key' => 'field_65e2a1b3c4d71',
					'label' => __( 'Link Title', 'lesverts' ),
					'name' => 'title',
					'aria-label' => '',
					'type' => 'text',
					'instructions' => '',
					'required' => 0,
					'conditional_logic' => 0,
					'wrapper' => array(
						'width' => '',
						'class' => '',
						'id' => '',
					),
					'default_value' => '',
					'translations' => 'translate',
					'maxlength' => '',
					'allow_in_bindings' => 0,
					'placeholder' => '',
					'prepend' => '',
					'append' => '',
				),
				array(
					'key' => 'field_65e2a1b3c4d72',
					'label' => __( 'Link URL', 'lesverts' ),
					'name' => 'url',
					'aria-label' => '',
					'type' => 'page_link',
					'instructions' => '',
					'required' => 0,
					'conditional_logic' => 0,
					'wrapper' => array(
						'width' => '',
						'class' => '',
						'id' => '',
					),
					'post_type' => '',
					'post_status' => '',
					'taxonomy' => '',
					'allow_archives' => 1,
					'multiple' => 0,
					'translations' => 'copy_once',
					'allow_null' => 0,
					'allow_in_bindings' => 0,
				),
			),
		),
	),
	'location' => array(
		array(
			array(
				'param' => 'page_type',
				'operator' => '==',
				'value' => 'front_page',
			),
		),
	),
	'menu_order' => 0,
	'position' => 'normal',
	'style' => 'default',
	'label_placement' => 'top',
	'instruction_placement' => 'label',
	'hide_on_screen' => '',
	'active' => true,
	'description' => '',
	'show_in_rest' => false,
	'display_title' => '',
	'modified' => 1721100000,
) );
} );

endif;
