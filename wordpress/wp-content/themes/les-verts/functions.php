...
class StarterSite extends TimberSite {
	...
	function supt_setup_assets() {
		// css
		wp_enqueue_style( 'screen', get_stylesheet_directory_uri() . '/static/style'. ( WP_DEBUG ? '' : '.min' ) .'.css', false, THEME_VERSION );
		if ( is_rtl() ) {
			wp_enqueue_style( 'rtl', get_stylesheet_directory_uri() . '/static/rtl'. ( WP_DEBUG ? '' : '.min' ) .'.css', false, THEME_VERSION );
		}
		...
		if ( !is_admin() ) {
			...
			if ( WP_DEBUG ) {
				...
				if (is_multisite()) { // Removing trailing slash to attach browser-sync
					wp_enqueue_script( 'bs', substr(network_site_url(), 0, -1) .':4000/browser-sync/browser-sync-client.js', false, null, true );
				} else {
					wp_enqueue_script( 'bs', get_site_url() .':4000/browser-sync/browser-sync-client.js', false, null, true );
				}
			}
		}
